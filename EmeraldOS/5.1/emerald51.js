...
(function () {
    ...
    const BUILD = {
        product: "EmeraldOS",
        version: "5.1",
        displayName: "EmeraldOS 5.1",
        codename: "Office, Sharing & Storage Update",
        fileLimit: 1024 * 1024
    };
    ...
    window.refreshFileSharing51 = async function () {
        ...
        const fileRows = Object.entries(files)
            .filter(([id, file]) => file.type !== "folder")
            .sort((a, b) => String(a[1].name).localeCompare(String(b[1].name)))
            .map(([id, file]) => `
                <div class="share-row">
                    <span><b>${esc(file.name || id)}</b><br><small>${esc(formatBytes(fileSizeFromMeta(file)))} · ${file.hasStorageBlob ? "Firebase Storage" : "Firestore"}</small></span>
    ...
    window.shareFilePrompt51 = async function (recipient = "", fileId = "") {
        try {
            const files = await loadDrive();
            const selectedFileId = fileId || prompt("File ID to share:", Object.keys(files)[0] || "");
