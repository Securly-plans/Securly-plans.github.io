...
function normalizeEdition(id) {
    return TESTOS_EDITION_LEVELS[id] ? id : "home";
}

function getActiveEdition() {
    return normalizeEdition(
        localStorage.getItem("testos_edition") || "home"
    );
}
...
/* =========================================================
   TESTOS 3.2.T.3 DEVELOPER EDITION PATCH
...
(function () {
        ...
        window.openWindow("Build Inspector", `
            <div style="padding:10px;">
                ...
                <div class="inset-panel">
                    <b>Build:</b> ${safeHTML(build.displayName || "TestOS 3.2.T.3")}<br>
                    <b>Version:</b> ${safeHTML(build.version || "3.2.T.3")}<br>
    ...
    /* -----------------------------------------------------
       TESTOS 3.2.T.3 ADVANCED DEVELOPER PATCH
       - More apps
       - Expanded terminal
       - HKEY-style registry
    ...
    const DEFAULT_TESTOS_REGISTRY = {
        ...
        "HKEY_TESTOS\\SOFTWARE\\TestOS\\CurrentVersion": "3.2.T.3",
    ...
    window.runCommand = async function(cmdLine = "") {
        ...
        try {
            ...
            if (command === "help") {
                ...
                result = "TestOS 3.2.T.3 Advanced Developer Build";
            }
                ...
                result = safeHTML(localStorage.getItem("testos_edition_name") || localStorage.getItem("testos_edition") || "TestOS Home");
            }
    ...
    window.openAudioNotes = function () {
        window.openWindow("Audio Notes", `<div class="app-panel"><h3>Audio Notes</h3><p>Audio note recorder placeholder for TestOS 3.2.T.3.</p><button onclick="notify('Audio Notes','Recorder initialized.',2500,'info')">Initialize Recorder</button></div>`, "audioNotes");
    };
    ...
    window.openScriptLab = function () {
        window.openWindow("Script Lab", `<div class="app-panel script-lab"><h3>Script Lab</h3><textarea id="script_lab_code" spellcheck="false">return 'Hello from TestOS 3.2.T.3';</textarea><button onclick="runScriptLab()">Run</button><pre id="script_lab_output"></pre></div>`, "scriptLab");
    };
