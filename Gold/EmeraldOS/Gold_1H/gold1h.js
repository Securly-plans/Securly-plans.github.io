...
========================================================= */
(function EmeraldOSGold1H(){
  if(window.__EMERALDOS_GOLD_1H_LOADED__) return;
  window.__EMERALDOS_GOLD_1H_LOADED__=true;

  const VERSION="1H";
  const FOLDER="Gold_1H";
  const PREFIX="gold1g_";
  const PUBLISHER_PIN="093013";
  const REGISTRY_KEY=PREFIX+"registry_1h";
  const REGISTRY_HISTORY_KEY=PREFIX+"registry_1h_history";
  const CLIPBOARD_KEY=PREFIX+"clipboard_history_1h";
  const STARTUP_KEY=PREFIX+"startup_apps_1h";
  const CLOUD_CATEGORIES=["prefs","files","tickets","mail","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","registry_1h","clipboard_history_1h","startup_apps_1h"];
  ...
  const writeJSON=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1H local write failed",key,error);return false}};
  ...
  function notify(title,body,app="EmeraldOS Gold 1H"){
    try{
  ...
  function isStaff(){
    ...
    const role2=(localStorage.getItem("role2")||"").toLowerCase();
    if(["admin","administrator","mod","moderator","operator","staff"].includes(role)) return true;
    if(["administrator","operator","staff"].includes(role2)) return true;
    for(const key of [PREFIX+"staff_session","gold_staff_session","emeraldStaffSession"]){
      const value=readJSON(key,null);
      if(value && value.verified!==false && (value.username||value.user||value.role)) return true;
    }
    return false;
  }
  ...
  function focusAnyWindow(win){
    ...
    const button=$("gold1h_task_"+win.id);
    qa(".gold1h-task-button").forEach(b=>b.classList.remove("active"));
    if(button) button.classList.add("active");
  }

  function taskButtonFor(win){
    const taskApps=$("taskApps");
    if(!taskApps || $("gold1h_task_"+win.id)) return;
    const button=document.createElement("button");
    button.id="gold1h_task_"+win.id;
    button.className="gold1h-task-button active";
    button.title=win.dataset.title||"EmeraldOS Gold 1H";
  ...
  function saveCustomWindowState(){
    const state=qa(".gold1h-window").map(w=>({
      id:w.id,title:w.dataset.title,left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,
      minimized:w.classList.contains("minimized"),maximized:w.classList.contains("gold1h-maximized")
    }));
    writeJSON(PREFIX+"window_state_1h",state);
  }

  function open1HWindow(id,title,html,options={}){
    ...
    win.className="window gold1h-window";
  ...
  function wireCustomWindow(win){
    ...
    title?.addEventListener("pointerdown",e=>{
      if(e.target.closest("button")||win.classList.contains("gold1h-maximized")) return;
    ...
    win.querySelector("[data-g1h-resize]")?.addEventListener("pointerdown",e=>{
      if(win.classList.contains("gold1h-maximized")) return;
  ...
  function minimizeWindow(id){
    ...
    $("gold1h_task_"+id)?.classList.remove("active");
    saveCustomWindowState();
  }

  function maximizeWindow(id){
    const win=$(id);if(!win)return;
    if(win.classList.contains("gold1h-maximized")){
      const restore=readJSON(PREFIX+"restore_"+id,null);
      win.classList.remove("gold1h-maximized","gold1h-snapped");
      ...
      win.classList.add("gold1h-maximized");
    }
  ...
  function closeWindow(id){
    const win=$(id);if(!win)return;
    $("gold1h_task_"+id)?.remove();
    customWindows.delete(id);win.remove();saveCustomWindowState();
  }

  function snapWindow(win,zone){
    if(!win)return;
    if(win.classList.contains("gold1h-maximized")) win.classList.remove("gold1h-maximized");
    ...
    win.classList.add("gold1h-snapped");
    const map={
  ...
  function showSnapPreview(x,y){
    ...
    let preview=$("gold1hSnapPreview");
    if(!zone){preview?.remove();return}
    if(!preview){preview=document.createElement("div");preview.id="gold1hSnapPreview";preview.className="gold1h-snap-preview";document.body.appendChild(preview)}
  ...
  function hideSnapPreview(){$("gold1hSnapPreview")?.remove()}
  ...
  function registryTreeHTML(node,path=[],depth=0){
    return registryChildren(node).map(name=>{
      ...
      return `<div class="gold1h-tree-row ${selected?"selected":""}" data-reg-path="${esc(pathToken(next))}" style="padding-left:${depth*15}px">
        <span class="gold1h-tree-caret" data-reg-toggle="${esc(pathToken(next))}">${hasChildren?(expanded?"▼":"▶"):""}</span>
        <span class="gold1h-tree-folder">▰</span><span>${esc(name)}</span>
  ...
  function registryHTML(){
    ...
    return `<div class="gold1h-app gold1h-registry">
      <div class="gold1h-commandbar">
        ...
        <input id="gold1hRegistryPath" class="gold1h-field gold1h-reg-path gold1h-code" value="${esc(pathText(registrySelection))}" aria-label="Registry path">
        ${locked?'<span class="gold1h-reg-lock">Read-only without Staff Edition</span>':""}
      </div>
      <div class="gold1h-reg-body">
        <div class="gold1h-reg-tree">${registryTreeHTML(data)}</div>
        <div class="gold1h-reg-values">
          <table class="gold1h-reg-table"><thead><tr><th style="width:32%">Name</th><th style="width:19%">Type</th><th>Data</th></tr></thead>
          <tbody>${Object.entries(values).map(([name,value])=>`<tr data-reg-value="${esc(encodeURIComponent(name))}" class="${registrySelectedValue===name?"selected":""}"><td>${esc(name)}</td><td>${esc(value.type||"REG_SZ")}</td><td>${esc(registryValueDisplay(value))}</td></tr>`).join("")||'<tr><td colspan="3" class="gold1h-empty">This key has no values.</td></tr>'}</tbody></table>
        </div>
      </div>
      <div class="gold1h-statusbar"><span>${esc(pathText(registrySelection))}</span><span>${Object.keys(values).length} value(s)</span><span>${locked?"Protected hive":"Editable"}</span></div>
    </div>`;
  }
  function openRegistryEditor(){
    const win=open1HWindow("regedit","Emerald Registry Editor",registryHTML(),{width:1050,height:700,icon:"▦",refresh:true});
  ...
  function wireRegistryUI(win){
    ...
    const pathInput=$("gold1hRegistryPath");pathInput?.addEventListener("keydown",e=>{if(e.key==="Enter")navigateRegistry(pathInput.value)});
  }
  ...
  function applyRegistryEffects(data=registryData()){
    ...
    document.body.classList.toggle("gold1h-no-transparency",registryValueAt(data,personal,"Transparency",true)===false);
  ...
  function taskManagerHTML(tab="processes"){
    return `<div class="gold1h-app gold1h-taskmgr">
      <div class="gold1h-tabs"><button class="${tab==="processes"?"active":""}" data-task-tab="processes">Processes</button><button class="${tab==="performance"?"active":""}" data-task-tab="performance">Performance</button><button class="${tab==="startup"?"active":""}" data-task-tab="startup">Startup</button><button class="${tab==="details"?"active":""}" data-task-tab="details">Details</button></div>
      <div class="gold1h-taskmgr-content">${taskManagerPage(tab)}</div>
      <div class="gold1h-statusbar"><span>${processRows().length} apps and windows</span><span>EmeraldOS Gold 1H</span><span class="gold1h-muted">Virtual process manager</span></div>
    </div>`;
  }
  function taskManagerPage(tab){
    if(tab==="performance"){
      ...
      return `<div class="gold1h-performance-grid"><div class="gold1h-performance-card"><h3>CPU</h3><h1>${Math.min(94,8+processRows().length*4)}%</h1><div class="gold1h-meter"><span style="width:${Math.min(94,8+processRows().length*4)}%"></span></div><p class="gold1h-muted">Browser-hosted virtual CPU activity estimate.</p></div><div class="gold1h-performance-card"><h3>Memory</h3><h1>${percent}%</h1><div class="gold1h-meter"><span style="width:${percent}%"></span></div><p>${(localBytes/1024/1024).toFixed(2)} MB local VM storage in use.</p></div><div class="gold1h-performance-card"><h3>Cloud VM</h3><h2>${esc(localStorage.getItem(PREFIX+"cloud_status")||"Ready")}</h2><p>Path: emeraldOSUsers/${esc(userName())}/goldVM/current</p></div><div class="gold1h-performance-card"><h3>Uptime</h3><h2>${formatUptime(performance.now())}</h2><p>${navigator.onLine?"Network connected":"Offline"}</p></div></div>`;
    }
    if(tab==="startup"){
      ...
      return `<table class="gold1h-process-table"><thead><tr><th>Name</th><th>Status</th><th>Startup impact</th><th></th></tr></thead><tbody>${startup.map((a,i)=>`<tr><td>${esc(a.name)}</td><td>${a.enabled?"Enabled":"Disabled"}</td><td>${esc(a.impact||"Not measured")}</td><td><button class="gold1h-button" data-startup-toggle="${i}">${a.enabled?"Disable":"Enable"}</button></td></tr>`).join("")}</tbody></table>`;
    }
    const rows=processRows();
    return `<table class="gold1h-process-table"><thead><tr><th>Name</th><th>Status</th><th>App ID</th><th>Memory</th><th></th></tr></thead><tbody>${rows.map(p=>`<tr><td>${esc(p.title)}</td><td>${esc(p.status)}</td><td class="gold1h-code">${esc(p.app)}</td><td>${p.memory} MB</td><td><button class="gold1h-button" data-end-task="${esc(p.id)}">End task</button></td></tr>`).join("")||'<tr><td colspan="5" class="gold1h-empty">No applications are running.</td></tr>'}</tbody></table>`;
  }
  ...
  function openTaskManager(tab="processes"){
    const win=open1HWindow("taskmanager","Task Manager",taskManagerHTML(tab),{width:860,height:590,icon:"▤",refresh:true});wireTaskManager(win);return win;
  }
  ...
  function controlPanelHTML(){return `<div class="gold1h-app gold1h-control-shell"><div class="gold1h-control-header"><h2>Control Panel</h2><p class="gold1h-muted">Adjust your computer's settings.</p><input id="gold1hControlSearch" class="gold1h-field" style="width:min(420px,100%)" placeholder="Search Control Panel"></div><div class="gold1h-control-grid">${CONTROL_ITEMS.map(item=>`<button class="gold1h-control-item" data-control="${item[3]}" data-control-text="${esc((item[0]+" "+item[1]).toLowerCase())}"><span class="icon">${item[2]}</span><span><b>${esc(item[0])}</b><small>${esc(item[1])}</small></span></button>`).join("")}</div></div>`}
  function openControlPanel(){const win=open1HWindow("control","Control Panel",controlPanelHTML(),{width:920,height:650,icon:"⚙",refresh:true});wireControlPanel(win);return win}
  function wireControlPanel(win){
    ...
    $("gold1hControlSearch")?.addEventListener("input",e=>{const q=e.target.value.toLowerCase();win.querySelectorAll("[data-control]").forEach(b=>b.style.display=b.dataset.controlText.includes(q)?"":"none")});
  }
  function openControlItem(id){
    ...
    if(id==="network")return openSimplePanel("Network and Internet",`<h2>Network status</h2><p>${navigator.onLine?"Connected to the Internet":"Offline"}</p><div class="gold1h-control-grid"><button class="gold1h-control-item" onclick="Gold1H.runCommand('settings')"><span class="icon">⌁</span><span><b>Network settings</b><small>Open EmeraldOS Settings.</small></span></button></div>`);
    if(id==="hardware")return base("settings");
  }
  function openSimplePanel(id,html){return open1HWindow(id.toLowerCase().replace(/\W/g,"_"),id,`<div class="gold1h-app" style="padding:18px;overflow:auto">${html}</div>`,{width:760,height:520,icon:"⚙",refresh:true})}
  function openSystemInformation(){
    ...
    return openSimplePanel("System",`<h1>EmeraldOS Gold 1H</h1><p>Emerald Systems virtual operating environment</p><hr><div class="gold1h-control-grid"><div class="gold1h-performance-card"><b>Edition</b><p>EmeraldOS Gold 1H</p></div><div class="gold1h-performance-card"><b>User</b><p>${esc(userName())}</p></div><div class="gold1h-performance-card"><b>Browser platform</b><p>${esc(navigator.platform||"Web platform")}</p></div><div class="gold1h-performance-card"><b>Display</b><p>${innerWidth} × ${innerHeight} @ ${devicePixelRatio||1}x</p></div><div class="gold1h-performance-card"><b>Local VM storage</b><p>${(storage/1024/1024).toFixed(2)} MB</p></div><div class="gold1h-performance-card"><b>Cloud VM</b><p>emeraldOSUsers/${esc(userName())}/goldVM/current</p></div></div>`);
  }

  /* ------------------------- Run and Clipboard ------------------------- */
  function openRunDialog(){
    const html=`<div class="gold1h-app"><div class="gold1h-run"><div class="gold1h-run-icon">▦</div><div><p>Type the name of a program, folder, document, or EmeraldOS command, and Gold will open it for you.</p><div class="gold1h-run-row"><label for="gold1hRunInput">Open:</label><input id="gold1hRunInput" class="gold1h-field" autocomplete="off"></div></div></div><div class="gold1h-run-buttons"><button class="gold1h-button primary" id="gold1hRunOK">OK</button><button class="gold1h-button" onclick="Gold1H.closeWin('win_g1h_run')">Cancel</button><button class="gold1h-button" onclick="Gold1H.runCommand('explorer')">Browse...</button></div></div>`;
    const win=open1HWindow("run","Run",html,{width:520,height:255,icon:"▦",refresh:true});
    const input=$("gold1hRunInput");setTimeout(()=>input?.focus(),0);input?.addEventListener("keydown",e=>{if(e.key==="Enter")runCommand(input.value)});$("gold1hRunOK")?.addEventListener("click",()=>runCommand(input?.value||""));return win;
  }
  ...
  function openClipboardHistory(){
    ...
    const html=`<div class="gold1h-app"><div class="gold1h-commandbar"><b>Clipboard</b><span class="gold1h-muted">Win + V</span><span style="flex:1"></span><button id="gold1hClipboardClear">Clear all</button></div><div class="gold1h-clipboard-list">${items.map(item=>`<div class="gold1h-clipboard-item" data-clip-id="${esc(item.id)}" title="Click to copy">${esc(item.text)}</div>`).join("")||'<div class="gold1h-empty"><h2>Your clipboard is empty</h2><p>Copy text to add it to clipboard history.</p></div>'}</div></div>`;
    const win=open1HWindow("clipboard","Clipboard history",html,{width:410,height:540,icon:"▤",refresh:true});
    ...
    $("gold1hClipboardClear")?.addEventListener("click",()=>{writeJSON(CLIPBOARD_KEY,[]);openClipboardHistory()});return win;
  }
  ...
  function calendarHTML(date=new Date()){
    ...
    const labels=["Su","Mo","Tu","We","Th","Fr","Sa"].map(d=>`<span class="gold1h-muted">${d}</span>`);
    ...
    return `<div class="gold1h-calendar-head"><b>${date.toLocaleString(undefined,{month:"long",year:"numeric"})}</b><span>Today</span></div><div class="gold1h-calendar-grid">${labels.join("")}${days.join("")}</div>`;
  }
  function toggleClockFlyout(){
    const existing=$("gold1hClockFlyout");if(existing){existing.remove();return}
    const date=new Date(),fly=document.createElement("section");fly.id="gold1hClockFlyout";fly.className="gold1h-clock-flyout";
    fly.innerHTML=`<div class="gold1h-clock-time">${date.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div><div class="gold1h-clock-date">${date.toLocaleDateString([],{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</div>${calendarHTML(date)}`;
  ...
  async function fetchLatestManifest(){
    ...
    }catch(error){console.warn("Gold 1H update check used local manifest",error);return fallback}
  }
  async function openUpdateCenter(latestManifest=null){
    ...
    const html=`<div class="gold1h-app" style="padding:20px;overflow:auto"><div style="display:flex;align-items:start;justify-content:space-between;gap:12px"><div><h1>System Update</h1><p class="gold1h-muted">E.L.S.U.S. changes this VM only after you choose to update.</p></div><button id="gold1hCheckUpdates" class="gold1h-button primary">Check for updates</button></div><div class="gold1h-performance-grid"><div class="gold1h-performance-card"><h3>This VM</h3><p><b>EmeraldOS Gold 1H</b></p><p>Gold_1H / OS.html</p></div><div class="gold1h-performance-card"><h3>Latest configured release</h3><p><b>${esc(latest.releaseTitle||`EmeraldOS Gold ${latestVersion}`)}</b></p><p>${esc(latest.folder||"")} / ${esc(latest.entry||"OS.html")}</p></div><div class="gold1h-performance-card"><h3>Status</h3><p>${newer?"A newer release is available.":"This VM is on the latest configured release."}</p></div><div class="gold1h-performance-card"><h3>VM continuity</h3><p>Files, apps, settings, registry, and workspace data remain in the version-independent Gold VM.</p></div></div><h3>Release information</h3><p>${esc(latest.summary||"No release information is available.")}</p><div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:18px"><button id="gold1hSaveBeforeUpdate" class="gold1h-button">Save cloud VM</button><button id="gold1hApplyUpdate" class="gold1h-button primary" ${newer?"":"disabled"}>Update this VM now</button><button class="gold1h-button" onclick="Gold1H.openPublisherManager()">Update Publisher Manager</button></div></div>`;
    const win=open1HWindow("updateshell","System Update",html,{width:980,height:680,icon:"↻",refresh:true});
    $("gold1hCheckUpdates")?.addEventListener("click",async()=>openUpdateCenter(await fetchLatestManifest()));
    $("gold1hSaveBeforeUpdate")?.addEventListener("click",()=>saveWorkspaceNow1H(true));
    $("gold1hApplyUpdate")?.addEventListener("click",()=>applyConfiguredUpdate(latest));return win;
  }
  async function applyConfiguredUpdate(manifest){
    const latestVersion=manifest?.latestVersion||manifest?.build||"";
    if(versionRank(latestVersion)<=versionRank(VERSION))return notify("No newer update","EmeraldOS Gold 1H is already current.","System Update");
    await saveWorkspaceNow1H(false);
  ...
  /* ------------------------- Publisher ------------------------- */
  function releaseManifest(){return {product:"EmeraldOS Gold",latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",channel:"stable",status:"stable",required:false,enabled:true,setupMode:"continue",releaseTitle:"EmeraldOS Gold 1H",summary:"EmeraldOS Gold 1H improves the desktop, Start, taskbar, window management, system tools, user friendliness, virtual HKEY registry, cloud VM storage reliability, and accessibility.",migrationFrom:["1G","1F","1E","1D"],migrationId:"gold1g-to-gold1h-windows10-ux",minShellVersion:"1.0",rollbackFolder:"Gold_1G",rollbackVersion:"1G",releasedAt:now()}}
  ...
  function openPublisherManager(){
    if(!isStaff()){
      ...
      return openSimplePanel("Staff verification required",`<h2>Update Publisher Manager</h2><p>Publishing is available only from a verified Gold Staff Edition session.</p><button class="gold1h-button primary" onclick="location.href='staff.html'">Open Gold Staff Edition</button>`);
    }
    const unlocked=publisherUnlocked();
    const html=`<div class="gold1h-app" style="padding:20px;overflow:auto"><h1>Update Publisher Manager</h1><p>Publish <b>EmeraldOS Gold 1H</b> to <code>system/emeraldGoldLatest</code>.</p><div class="gold1h-performance-card"><b>Security status</b><p>${unlocked?"Publisher PIN accepted for this staff session.":"Locked. Enter the publisher PIN."}</p>${unlocked?"":'<label>Publisher PIN <input id="gold1hPublisherPin" type="password" class="gold1h-field"></label> <button id="gold1hUnlockPublisher" class="gold1h-button">Unlock</button>'}</div><div class="gold1h-performance-card"><b>Release target</b><p>Gold_1H / OS.html</p><p>Release title: EmeraldOS Gold 1H</p></div><div style="display:flex;gap:8px;margin-top:18px"><button id="gold1hPublishVersion" class="gold1h-button primary" ${unlocked?"":"disabled"}>Publish this Version</button><button id="gold1hExportManifest" class="gold1h-button">Export manifest</button></div><p class="gold1h-muted">Gold 1H never publishes from a query string or during first boot.</p></div>`;
    const win=open1HWindow("publisher","Update Publisher Manager",html,{width:720,height:540,icon:"UP",refresh:true});
    $("gold1hUnlockPublisher")?.addEventListener("click",()=>unlockPublisher($("gold1hPublisherPin")?.value||""));
    $("gold1hPublishVersion")?.addEventListener("click",()=>publishLatest());
    $("gold1hExportManifest")?.addEventListener("click",()=>saveText("FIREBASE_EMERALDGOLDLATEST_1H.json",JSON.stringify(releaseManifest(),null,2),"application/json"));return win;
  }
  ...
  async function publishLatest(){
    ...
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"system","emeraldGoldLatest"),releaseManifest(),{merge:true});writeJSON("emeraldGoldShell_latest",releaseManifest());localStorage.setItem(PREFIX+"latest_manifest_published_1h","true");notify("E.L.S.U.S. updated","system/emeraldGoldLatest now points to Gold_1H.","System Update");return true}catch(error){console.warn(error);notify("Publish failed",error.message,"System Update");return false}
  }

  /* ------------------------- Split Cloud VM ------------------------- */
  function localCategoryValue(category){
    if(category==="registry_1h")return registryData();
    if(category==="clipboard_history_1h")return clipboardHistory();
    if(category==="startup_apps_1h")return readJSON(STARTUP_KEY,[]);
  ...
  function currentWindowState(){return qa(".window").map(w=>({id:w.id,title:w.dataset.title||"",left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,minimized:w.classList.contains("minimized"),maximized:w.classList.contains("max")||w.classList.contains("gold1h-maximized")})).slice(0,80)}
  async function saveWorkspaceNow1H(show=true){
    const summary={product:"EmeraldOS Gold",version:VERSION,build:VERSION,activeVersion:VERSION,activeFolder:FOLDER,schema:"gold1h-split-v1",user:userName(),savedAt:now(),categories:CLOUD_CATEGORIES,openWindows:currentWindowState(),cloudPath:"emeraldOSUsers/{username}/goldVM/current"};
    writeJSON(PREFIX+"workspace_meta_1h",summary);
    try{
      const fbm=await import("./firebase.js");
      ...
      if(show)notify("Cloud VM saved","Gold 1H saved a split, quota-safe VM state to the cloud.","Gold VM");return summary;
    }catch(error){
      console.warn("Gold 1H split cloud save failed",error);localStorage.setItem(PREFIX+"cloud_status","save failed: "+error.message);if(show)notify("Saved locally","Cloud sync will retry when Firebase is available.","Gold VM");return summary;
    }
  }
  async function restoreWorkspace1H(show=true){
    try{
      const fbm=await import("./firebase.js"),snap=await fbm.getDoc(fbm.doc(fbm.db,"emeraldOSUsers",userName(),"goldVM","current"));
      if(!snap.exists()){if(show)notify("No cloud VM state","No Gold VM state was found for this account.","Gold VM");return null}
      const summary=snap.data();
      if(summary.schema!=="gold1h-split-v1"){
        Object.entries(summary).forEach(([key,value])=>{if(CLOUD_CATEGORIES.includes(key))writeJSON(PREFIX+key,value)});
        if(show)notify("Legacy VM restored","Gold 1H restored the existing Gold cloud VM state.","Gold VM");return summary;
      }
      for(const category of summary.categories||CLOUD_CATEGORIES){
        const value=await cloudReadCategory(fbm,category);if(value===undefined)continue;
        const key=category==="registry_1h"?REGISTRY_KEY:category==="clipboard_history_1h"?CLIPBOARD_KEY:category==="startup_apps_1h"?STARTUP_KEY:PREFIX+category;
        writeJSON(key,value);
      }
      applyRegistryEffects();localStorage.setItem(PREFIX+"cloud_status","restored");localStorage.setItem(PREFIX+"cloud_last_restore",now());
      try{window.Gold50?.setPrefs?.(readJSON(PREFIX+"prefs",{}))}catch{}
      if(show)notify("Cloud VM restored","Apps, files, settings, registry, and workspace data were restored.","Gold VM");return summary;
    }catch(error){console.warn("Gold 1H cloud restore failed",error);if(show)notify("Restore failed",error.message,"Gold VM");return null}
  }
  let saveDebounce=null;function saveWorkspaceDebounced1H(){clearTimeout(saveDebounce);saveDebounce=setTimeout(()=>saveWorkspaceNow1H(false),1200)}

  /* ------------------------- Shell enhancements ------------------------- */
  function customToolButtons(){return `
    <button class="gold1h-start-tile" onclick="Gold1H.openRegistryEditor()"><span>▥</span><b>Registry Editor</b></button>
    <button class="gold1h-start-tile" onclick="Gold1H.openTaskManager()"><span>▤</span><b>Task Manager</b></button>
    <button class="gold1h-start-tile" onclick="Gold1H.openControlPanel()"><span>⚙</span><b>Control Panel</b></button>
    <button class="gold1h-start-tile" onclick="Gold1H.openRun()"><span>▦</span><b>Run</b></button>
    <button class="gold1h-start-tile" onclick="Gold1H.openClipboardHistory()"><span>▧</span><b>Clipboard</b></button>
    <button class="gold1h-start-tile" onclick="Gold1H.openPublisherManager()"><span>UP</span><b>Publisher</b></button>`}
  function enhanceStartMenu(){
    const menu=$("startMenu");if(!menu||startEnhancing||menu.querySelector(".gold1h-start-shell"))return;
    startEnhancing=true;
    const original=menu.innerHTML||'<div class="gold1h-empty">Applications are loading.</div>';
    menu.innerHTML=`<div class="gold1h-start-shell"><div class="gold1h-start-rail"><button title="User">◉</button><button title="Documents" onclick="Gold1H.runCommand('explorer')">▤</button><button title="Pictures" onclick="Gold1H.runCommand('explorer')">▧</button><div class="rail-spacer"></div><button title="Settings" onclick="Gold1H.runCommand('settings')">⚙</button><button title="Power" onclick="Gold1H.showPowerMenu()">⏻</button></div><div class="gold1h-start-apps"><h3>All apps</h3><div class="gold1h-start-original">${original}</div></div><div class="gold1h-start-tools"><h3>Gold 1H system tools</h3><div class="gold1h-start-tool-grid">${customToolButtons()}</div></div></div>`;
    startEnhancing=false;
  }
  function enhanceActionCenter(){
    const panel=$("actionCenter");if(!panel||actionEnhancing||panel.querySelector(".gold1h-quick-actions"))return;
    actionEnhancing=true;
    panel.insertAdjacentHTML("afterbegin",`<div class="gold1h-quick-actions"><button class="gold1h-quick-action" data-quick="focus"><span>☾</span>Focus assist</button><button class="gold1h-quick-action" data-quick="night"><span>◐</span>Night light</button><button class="gold1h-quick-action" data-quick="settings"><span>⚙</span>All settings</button><button class="gold1h-quick-action" data-quick="save"><span>☁</span>Cloud save</button><button class="gold1h-quick-action" data-quick="safe"><span>!</span>Safe mode</button><button class="gold1h-quick-action" data-quick="registry"><span>▥</span>Registry</button><button class="gold1h-quick-action" data-quick="task"><span>▤</span>Task Manager</button><button class="gold1h-quick-action" data-quick="control"><span>▦</span>Control Panel</button></div>`);
  ...
  function quickAction(action,button){
    if(action==="focus"){document.body.classList.toggle("gold1h-focus");button.classList.toggle("active");return}
    if(action==="night"){document.body.classList.toggle("gold1h-night");button.classList.toggle("active");document.body.style.filter=document.body.classList.contains("gold1h-night")?"sepia(.22) brightness(.92)":"";return}
    if(action==="settings")return runCommand("settings");if(action==="save")return saveWorkspaceNow1H(true);if(action==="registry")return openRegistryEditor();if(action==="task")return openTaskManager();if(action==="control")return openControlPanel();
    if(action==="safe"&&confirm("Restart EmeraldOS Gold 1H in Safe Mode?")){localStorage.setItem(PREFIX+"safemode","true");location.reload()}
  }
  function enhanceSearch(){
    const panel=$("searchPanel");if(!panel||panel.querySelector(".gold1h-search-tools"))return;
    const box=document.createElement("div");box.className="gold1h-search-tools";box.innerHTML=`<h3>System tools</h3><div class="gold1h-start-tool-grid">${customToolButtons()}</div>`;panel.appendChild(box);
  }
  function enhanceDesktop(){
    ...
    tools.forEach(tool=>{if(desktop.querySelector(`[data-gold1h-shortcut="${tool.id}"]`))return;const button=document.createElement("button");button.className="desktop-icon gold1h-system-shortcut";button.dataset.gold1hShortcut=tool.id;button.innerHTML=`<span class="app-icon">${tool.icon}</span><span>${esc(tool.name)}</span>`;button.addEventListener("dblclick",tool.open);button.addEventListener("click",()=>{qa(".desktop-icon").forEach(x=>x.classList.remove("selected"));button.classList.add("selected")});desktop.appendChild(button)});
  }
  function showPowerMenu(){
    open1HWindow("power","Power",`<div class="gold1h-app" style="padding:20px"><h2>Power</h2><div class="gold1h-control-grid"><button class="gold1h-control-item" onclick="location.reload()"><span class="icon">↻</span><span><b>Restart</b><small>Restart EmeraldOS Gold 1H.</small></span></button><button class="gold1h-control-item" onclick="Gold1H.saveWorkspaceNow(true)"><span class="icon">☁</span><span><b>Save cloud VM</b><small>Save before leaving.</small></span></button><button class="gold1h-control-item" onclick="Gold50.logoutGold?.()"><span class="icon">⇥</span><span><b>Sign out</b><small>Close this user session.</small></span></button></div></div>`,{width:540,height:400,icon:"⏻",refresh:true});
  }
  function showDesktop(){
    const windows=qa(".window");const anyVisible=windows.some(w=>!w.classList.contains("minimized"));windows.forEach(w=>w.classList.toggle("minimized",anyVisible));qa(".gold1h-task-button").forEach(b=>b.classList.toggle("active",!anyVisible));
  }
  ...
  function showAltTab(){
    ...
    let overlay=$("gold1hAltTab");if(!overlay){overlay=document.createElement("div");overlay.id="gold1hAltTab";overlay.className="gold1h-alt-tab";document.body.appendChild(overlay)}
    overlay.innerHTML=`<div class="gold1h-alt-tab-inner">${altTabWindows.map((w,i)=>`<div class="gold1h-alt-tab-item ${i===altTabIndex?"active":""}"><span>${esc(w.dataset.icon||"▦")}</span><b>${esc(w.dataset.title||"Application")}</b><small>${w.classList.contains("minimized")?"Minimized":"Running"}</small></div>`).join("")}</div>`;
  }
  function commitAltTab(){
    $("gold1hAltTab")?.remove();const win=altTabWindows[altTabIndex];if(win){win.classList.remove("minimized");focusAnyWindow(win)}
  }
  ...
  /* ------------------------- Update setup ------------------------- */
  function updateSetupKey(){return `emeraldos_gold_1h_update_setup_done_${normalizeUser(userName())}`}
  ...
  async function finishUpdateSetup(){
    ...
    $("gold1hUpdateSetup")?.remove();
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"emeraldOSUsers",userName(),"goldVM","setup"),{firstBootCompleted:true,firstBootScope:"global-elsus-user",lastCompletedVersion:VERSION,lastUpdateSetupVersion:VERSION,lastSetupType:"update",licenseAccepted:true,licenseVersion:"Gold_1H",folder:FOLDER,updatedAt:now()},{merge:true})}catch(error){console.warn("Gold 1H update setup cloud save failed",error)}
    notify("EmeraldOS Gold 1H is ready","Your existing cloud VM data and preferences were preserved.","Setup");
  }
  function maybeShowUpdateSetup(){
    ...
    const overlay=document.createElement("section");overlay.id="gold1hUpdateSetup";overlay.className="gold1h-update-setup";overlay.innerHTML=`<div class="gold1h-update-card"><h1>Welcome to EmeraldOS Gold 1H</h1><p>This update keeps your existing E.L.S.U.S. cloud VM and introduces a more familiar, user-friendly desktop.</p><ul><li>Windows 10-inspired Start, taskbar, Action Center, and window controls</li><li>Emerald Registry Editor with HKEY hives and restore points</li><li>Window snapping, Alt + Tab, Win + R, Win + V, and Task Manager shortcuts</li><li>Control Panel, Run, Clipboard History, and improved system tools</li><li>Quota-safe split cloud VM storage</li></ul><p>No files or applications are removed.</p><div class="actions"><button class="gold1h-button" onclick="document.getElementById('gold1hUpdateSetup').remove()">Remind me later</button><button class="gold1h-button primary" onclick="Gold1H.finishUpdateSetup()">Continue</button></div></div>`;document.body.appendChild(overlay);
  }
  ...
  function patchGoldAPI(){
    ...
    if(!api.__gold1hOriginalOpenApp)api.__gold1hOriginalOpenApp=api.openApp?.bind(api);
    api.openApp=function gold1HOpenApp(id){
      ...
      if(custom[id])return custom[id]();return api.__gold1hOriginalOpenApp?.(id);
    };
    api.saveWorkspaceNow=saveWorkspaceNow1H;api.restoreWorkspace=restoreWorkspace1H;
    Object.assign(api,{openRegistryEditor1H:openRegistryEditor,openTaskManager1H:openTaskManager,openControlPanel1H:openControlPanel,openRun1H:openRunDialog,openClipboardHistory1H:openClipboardHistory,openUpdatePublisherManager1H:openPublisherManager,gold1hPublishLatest:publishLatest,gold1hManifest:releaseManifest});
    window.Gold1G=api;window.Gold1H=publicAPI;return true;
  }
  ...
  function installObserver(){
    const observer=new MutationObserver(()=>{clearTimeout(window.__gold1hRefreshTimer);window.__gold1hRefreshTimer=setTimeout(refreshEnhancements,40)});observer.observe(document.body,{childList:true,subtree:true});
  }
  function branding(){
    document.title="EmeraldOS Gold 1H";document.body.classList.add("gold1h","gold1h-shell");document.body.dataset.build=VERSION;
    qa("h1,h2,p,span").forEach(el=>{if(el.children.length===0&&el.textContent?.includes("EmeraldOS Gold 1G"))el.textContent=el.textContent.replaceAll("EmeraldOS Gold 1G","EmeraldOS Gold 1H")});
  }

  const publicAPI={
    version:VERSION,folder:FOLDER,openWindow:open1HWindow,closeWin:closeWindow,minimizeWin:minimizeWindow,maximizeWin:maximizeWindow,snapWindow,
    ...
    openUpdateCenter,openPublisherManager,unlockPublisher,publishLatest,manifest:releaseManifest,saveWorkspaceNow:saveWorkspaceNow1H,restoreWorkspace:restoreWorkspace1H,
    finishUpdateSetup,showPowerMenu,showDesktop,notify
  };
  window.Gold1H=publicAPI;
  ...
  function init(){
    ...
    setInterval(()=>saveWorkspaceNow1H(false),60000);
    window.addEventListener("beforeunload",()=>{writeJSON(PREFIX+"workspace_meta_1h",{version:VERSION,folder:FOLDER,savedAt:now(),openWindows:currentWindowState()})});
