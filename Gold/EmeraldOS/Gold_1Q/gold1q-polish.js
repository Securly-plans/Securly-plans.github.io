...
========================================================= */
(function EmeraldOSGold1QPolish(){
  if(window.__EMERALDOS_GOLD_1Q_POLISH__)return;
  window.__EMERALDOS_GOLD_1Q_POLISH__=true;

  const VERSION="1Q",PREFIX="gold1g_";
  const SESSION_KEY=PREFIX+"session_state_1q";
  const NOTIFICATION_PREFS_KEY=PREFIX+"notification_preferences_1q";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1Q continuity cache write failed",key,error);return false}};
  ...
  function captureSessionState(){
    const windows=qa(".window").filter(win=>win.isConnected).map(win=>({
      ...
      maximized:win.classList.contains("max")||win.classList.contains("gold1q-maximized"),
    ...
    const state={
      ...
      windows,
      virtualDesktop:read(PREFIX+"virtual_desktop_1q",read(PREFIX+"virtual_desktop_1o",1)),
  ...
  function previousSessionState(){
    ...
    const meta=read(PREFIX+"workspace_meta_1o",null)||read(PREFIX+"workspace_meta_1q",null);
  ...
  async function waitForMigration(){
    ...
    while(window.__GOLD1Q_MIGRATION_BLOCK_SAVES__===true&&Date.now()-started<maximumWait)await new Promise(resolve=>setTimeout(resolve,300));
  }

  async function restorePreviousWorkspace(){
    if(sessionStorage.getItem("gold1q_workspace_restore_attempted")==="true")return 0;
    sessionStorage.setItem("gold1q_workspace_restore_attempted","true");
    await waitForMigration();
    if(api()?.prefs?.().resumeWorkspace===false)return 0;
    if($("setupWizard")&&!$("setupWizard").classList.contains("hidden"))return 0;
    if($("gold1qUpdateSetup"))return 0;
    ...
    for(const id of apps){
      ...
      try{api()?.openApp?.(id)}catch(error){console.debug("Gold 1Q skipped workspace app",id,error?.message)}
  ...
  function ensureDimmer(){
    let dimmer=$("gold1qDisplayDimmer");
    if(!dimmer){dimmer=document.createElement("div");dimmer.id="gold1qDisplayDimmer";dimmer.className="gold1q-display-dimmer";dimmer.setAttribute("aria-hidden","true");document.body.appendChild(dimmer)}
    return dimmer;
  }
  function applyDevicePreferences(){
    ...
    document.querySelectorAll("audio,video").forEach(media=>{if(!media.dataset.gold1qVolumeApplied){media.volume=volume;media.dataset.gold1qVolumeApplied="true"}});
  }
  ...
  function openStartupContinuity(){
    ...
    const html=`<div class="app-shell gold1q-continuity-settings"><div class="app-toolbar"><button id="continuitySave" class="button primary">Save workspace now</button><button id="continuityClear" class="button">Clear saved window layout</button></div><div class="app-body"><div class="gold1q-page-title"><div><h1>Startup &amp; Continuity</h1><p>Control quiet startup and the cloud-based pick-up-where-you-left-off experience.</p></div><span class="gold1q-status-chip good">Cloud VM</span></div><section class="card"><h2>Workspace resume</h2><div class="setting-row"><span><b>Resume my workspace</b><small>Reopen safe applications and restore window positions after the VM has migrated.</small></span><input id="continuityResume" type="checkbox" ${p.resumeWorkspace!==false?"checked":""}></div><div class="setting-row"><span><b>Restore minimized windows</b><small>Otherwise restored applications open normally so they are easy to find.</small></span><input id="continuityMinimized" type="checkbox" ${p.restoreMinimizedWindows===true?"checked":""}></div><p class="muted">Last saved workspace: ${state?.updatedAt?new Date(state.updatedAt).toLocaleString():"No saved layout"}</p></section><section class="card"><h2>Notification calm mode</h2><div class="setting-row"><span><b>Quiet startup</b><small>Store startup events in Action Center without showing repeated banners.</small></span><input id="continuityQuiet" type="checkbox" ${n.quietStartup!==false?"checked":""}></div><div class="setting-row"><span><b>One startup summary</b><small>Show one combined banner after restoration finishes.</small></span><input id="continuitySummary" type="checkbox" ${n.showStartupSummary!==false?"checked":""}></div><div class="setting-row"><span><b>Update prompt only</b><small>Do not show a second toast when the update dialog is already visible.</small></span><input id="continuityUpdateOnly" type="checkbox" ${n.updatePromptOnly!==false?"checked":""}></div><div class="setting-row"><span><b>Quiet-start period</b><small>How long noncritical startup banners remain quiet.</small></span><select id="continuityQuietSeconds"><option value="10" ${n.startupQuietSeconds===10?"selected":""}>10 seconds</option><option value="18" ${n.startupQuietSeconds===18?"selected":""}>18 seconds</option><option value="30" ${n.startupQuietSeconds===30?"selected":""}>30 seconds</option></select></div></section><section class="card"><h2>Continuity safeguards</h2><p>Gold 1Q saves this session as a normal VM category. Migration remains blocked from cloud autosave until previous files, settings, preferences, Registry data, mail, apps, and session state are validated.</p></section></div></div>`;
    ...
    win.querySelector("#continuitySave").onclick=async()=>{save();await window.Gold1Q?.saveWorkspaceNow?.(true)};
  ...
  function patchAPI(){
    const core=api();if(!core||core.__gold1qPolishPatched)return false;
    core.__gold1qPolishPatched=true;
    ...
    const originalSave=window.Gold1Q?.saveWorkspaceNow?.bind(window.Gold1Q);
    if(originalSave&&!window.Gold1Q.__gold1qSessionSavePatched){window.Gold1Q.__gold1qSessionSavePatched=true;window.Gold1Q.saveWorkspaceNow=async show=>{captureSessionState();return originalSave(show)}}
    Object.assign(window.Gold1Q||core,{openStartupContinuity,captureSessionState,restorePreviousWorkspace,notificationPreferences,setNotificationPreferences});
  ...
  async function finishStartup(){
    ...
    sessionStorage.setItem("gold1q_startup_notification_phase_complete","true");
    const suppressed=Number(sessionStorage.getItem("gold1q_startup_notifications_suppressed"))||0;
    const prefs=notificationPreferences();
    if(prefs.showStartupSummary!==false&&!$("gold1qActiveUpdatePrompt")&&!$("gold1qUpdateSetup")&&(restored||suppressed)){
      api()?.notify?.("Welcome back",`${restored?`${restored} application${restored===1?"":"s"} restored. `:""}${suppressed?`${suppressed} startup notice${suppressed===1?" was":"s were"} kept in Action Center.`:"Your cloud VM is ready."}`,"EmeraldOS Gold 1Q");
    }
    captureSessionState();
  }
  ...
  function install(){
    ...
    const observer=new MutationObserver(()=>{applyDevicePreferences();clearTimeout(window.__gold1qClampTimer);window.__gold1qClampTimer=setTimeout(clampWindows,60)});
