...
========================================================= */
(function EmeraldOSGold1QExtras(){
  if(window.__EMERALDOS_GOLD_1Q_EXTRAS__) return;
  window.__EMERALDOS_GOLD_1Q_EXTRAS__=true;

  const VERSION="1Q";
  const FOLDER="Gold_1Q";
  const UPDATE_SERVICE="Original E.L.S.U.S.";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1q";
  const HISTORY_KEY=PREFIX+"update_history_1q";
  const SNOOZE_KEY=PREFIX+"update_snooze_1q";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1q";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1q";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1q";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1q";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1Q update check used local manifest cache",error?.message||error);
    }
  ...
  function updateButton(){
    let button=$("gold1qUpdateStatusBtn");
    if(button)return button;
    const taskbar=$("taskbar"),anchor=$("quickSettingsBtn")||$("shellStatusBtn");
    if(!taskbar||!anchor)return null;
    button=document.createElement("button");
    button.id="gold1qUpdateStatusBtn";
    button.className="task-btn gold1q-update-status hidden";
    button.title="System Update";
    button.setAttribute("aria-label","System Update");
    button.innerHTML='<span aria-hidden="true">↻</span><span class="gold1q-update-dot" aria-hidden="true"></span>';
  ...
  function removeUpdatePrompt(){
    $("gold1qActiveUpdatePrompt")?.remove();
    document.body.classList.remove("gold1q-update-prompt-open");
  }
  function showUpdatePrompt(manifest,force=false){
    ...
    const firstSetup=$("setupWizard"),updateSetup=$("gold1qUpdateSetup");
    ...
    prompt.id="gold1qActiveUpdatePrompt";
    prompt.className=`gold1q-active-update-prompt${manifest.required?" required":""}`;
    prompt.setAttribute("role","dialog");
    prompt.setAttribute("aria-labelledby","gold1qUpdatePromptTitle");
    const version=manifestVersion(manifest),title=manifest.releaseTitle||`EmeraldOS Gold ${version}`;
    prompt.innerHTML=`
      <div class="gold1q-update-prompt-head">
        <span class="gold1q-update-prompt-icon" aria-hidden="true">↻</span>
        <div><b id="gold1qUpdatePromptTitle">${esc(title)} is ready</b><small>E.L.S.U.S. update</small></div>
        <button id="gold1qUpdatePromptClose" aria-label="Close update notification">×</button>
      </div>
      ...
      <div class="gold1q-update-prompt-meta"><span>Current: ${VERSION}</span><span>Available: ${esc(version)}</span><span>${manifest.required?"Required update":"Optional update"}</span></div>
      <div class="gold1q-update-prompt-actions">
        <button id="gold1qUpdateNow" class="button primary">Update now</button>
        <button id="gold1qUpdateDetails" class="button">View details</button>
        <button id="gold1qUpdateLater" class="button">Later</button>
      </div>`;
    document.body.appendChild(prompt);
    document.body.classList.add("gold1q-update-prompt-open");
    localStorage.setItem(LAST_PROMPT_KEY,now());
    addHistory("prompt",manifest,"The active update prompt was displayed.");
    $("gold1qUpdateNow").onclick=()=>beginUserUpdate(manifest);
    $("gold1qUpdateDetails").onclick=()=>{removeUpdatePrompt();openUpdateStatus(manifest)};
    $("gold1qUpdateLater").onclick=()=>{snooze();removeUpdatePrompt();addHistory("snoozed",manifest,`Snoozed for ${updatePrefs().snoozeHours} hours.`);api()?.renderNotificationBadge?.()};
    $("gold1qUpdatePromptClose").onclick=()=>{if(!manifest.required)snooze(1);removeUpdatePrompt()};
    return true;
  }
  ...
  async function beginUserUpdate(manifest){
    ...
    try{
      if(window.Gold1Q?.requestVersionChange){await window.Gold1Q.requestVersionChange(manifest);return true}
      if(window.Gold1Q?.saveWorkspaceNow)await window.Gold1Q.saveWorkspaceNow(false);
      window.Gold1Q?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1Q live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1Q live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1Q?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1Q?.openUpdateCenter?.(currentManifest);
  ...
  function quickSettingsPanel(){
    let panel=$("gold1qQuickSettingsPanel");
    ...
    panel=document.createElement("section");panel.id="gold1qQuickSettingsPanel";panel.className="gold1q-quick-settings";
    panel.innerHTML=`<header><div><b>Quick Settings</b><small>EmeraldOS Gold 1Q</small></div><button id="quickSettingsClose" aria-label="Close Quick Settings">×</button></header><div class="gold1q-quick-grid"><button data-quick="focus" class="${prefs.focusAssist?"active":""}"><span>☾</span><b>Focus assist</b></button><button data-quick="night" class="${prefs.nightLight?"active":""}"><span>◐</span><b>Night light</b></button><button data-quick="dark" class="${prefs.theme==="dark"?"active":""}"><span>◑</span><b>Dark mode</b></button><button data-quick="transparent" class="${prefs.transparency!==false?"active":""}"><span>▧</span><b>Transparency</b></button><button data-quick="quiet" class="${notificationPrefs.quietStartup!==false?"active":""}"><span>◌</span><b>Quiet startup</b></button><button data-quick="updates" class="${updatePrefs().activeNotifications?"active":""}"><span>↻</span><b>Update alerts</b></button></div><div class="gold1q-quick-sliders"><label><span>Display</span><input id="quickBrightness" type="range" min="35" max="100" value="${Number(prefs.displayBrightness)||100}"><output>${Number(prefs.displayBrightness)||100}%</output></label><label><span>Volume</span><input id="quickVolume" type="range" min="0" max="100" value="${Number(prefs.systemVolume??80)}"><output>${Number(prefs.systemVolume??80)}%</output></label></div><footer><span class="gold1q-connectivity"><i class="${navigator.onLine?"online":"offline"}"></i>${navigator.onLine?"Connected":"Offline"}</span><div><button id="quickOpenUpdate">${updateAvailable?"Update available":"Up to date"}</button><button id="quickOpenSettings">All settings</button></div></footer>`;
  ...
  function openFocusSessions(){
    ...
    const html=`<div class="app-shell"><div class="app-toolbar"><b>Focus Sessions</b><span style="flex:1"></span><button id="focusSettings" class="button">Settings</button></div><div class="app-body gold1q-focus-app"><div class="gold1q-focus-clock" data-focus-time>${formatFocusTime(focusRemaining)}</div><h2 data-focus-state>${focusRunning?"Focusing":"Ready"}</h2><p>Use a distraction-free timer with Focus Assist and Gold notifications.</p><div class="gold1q-focus-controls"><button id="focusStart" class="button primary">${focusRunning?"Pause":"Start"}</button><button id="focusReset" class="button">Reset</button></div><div class="gold1q-focus-presets"><button data-focus-minutes="15">15 minutes</button><button data-focus-minutes="25">25 minutes</button><button data-focus-minutes="45">45 minutes</button><button data-focus-minutes="60">60 minutes</button></div><label class="gold1q-focus-option"><input id="focusAssistDuringSession" type="checkbox" checked> Turn on Focus Assist while the timer is running</label></div></div>`;
  ...
  function installEvents(){
    ...
    document.addEventListener("click",event=>{if(!event.target.closest("#gold1qQuickSettingsPanel")&&!event.target.closest("#quickSettingsBtn"))$("gold1qQuickSettingsPanel")?.remove()});
  ...
  function patchPublicAPI(){
    window.Gold1Q=window.Gold1Q||api()||{};
    Object.assign(window.Gold1Q,{elsusMode:"original",checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1Q","Update checks, Focus Sessions, and Quick Settings loaded.","System");
  }
