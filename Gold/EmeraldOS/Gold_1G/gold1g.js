"use strict";
/* EmeraldOS Gold 1G - independent logos, staff-only tools, live remote desktop, EmeraldOS Gold UX polish */
(function(){
const PREFIX="gold1g_";
const BUILD={name:"EmeraldOS Gold 1G",version:"1G",cloudPath:"goldVM/current",bios:"Emerald BIOS A9",shellManaged:true,folder:"Gold_1G"};

/* Gold 1G E.L.S.U.S. compatibility helpers.
   First-Boot Setup is now global to the signed-in user's E.L.S.U.S. VM, not per-version.
   Update Setup is per-version and appears after upgrading from an older E.L.S.U.S. compatible build. */
const ELSUS_SETUP_BASE="emeraldos_gold_elsus_first_setup_done";
const ELSUS_VERSION_BASE="emeraldos_gold_elsus_last_version";
function elsusUserKey(base){return base+"_"+String(username()).toLowerCase().replace(/[^a-z0-9._-]/g,"_")}
function elsusFirstSetupDone(){return localStorage.getItem(elsusUserKey(ELSUS_SETUP_BASE))==='true'}
function setElsusFirstSetupDone(){localStorage.setItem(elsusUserKey(ELSUS_SETUP_BASE),'true');localStorage.setItem(ELSUS_SETUP_BASE,'true')}
function elsusLastVersion(){return localStorage.getItem(elsusUserKey(ELSUS_VERSION_BASE))||localStorage.getItem(ELSUS_VERSION_BASE)||''}
function setElsusLastVersion(v){localStorage.setItem(elsusUserKey(ELSUS_VERSION_BASE),v);localStorage.setItem(ELSUS_VERSION_BASE,v)}
function migrateGold1ELocalData(){
  try{
    if(localStorage.getItem('gold1g_migrated_from_gold1e')==='true') return;
    const oldPrefix='gold1e_', newPrefix='gold1g_';
    ...
    if(localStorage.getItem('gold1e_setup_done')==='true'){
      localStorage.setItem('gold1g_updated_from','1E');
      setElsusFirstSetupDone();
    }
    // Update Setup must be run once for Gold 1G even if older data copied over.
...
function gold1gShouldRunFirstBootSetup(){return !elsusFirstSetupDone()}
...
function gold1gBootSetupGate(){
  if(gold1gShouldRunFirstBootSetup()) { setTimeout(openSetup,350); return; }
  localStorage.setItem(PREFIX+'setup_done','true');
  if(gold1gShouldRunUpdateSetup()) setTimeout(openUpdateSetup1G,900);
}
function gold1gStaffGateNotice(){
  notify('Staff Edition required','Sign in through Gold Staff Edition to use staff-only tools. Normal users will remain on the EmeraldOS Gold desktop.','Staff Edition');
  try{openStaffCenter()}catch(e){console.warn(e)}
}

/* EmeraldOS Gold 1G local data migration from Gold 1A.
   Cloud VM data remains version-independent under emeraldOSUsers/{username}/goldVM/current. */
function migrateGold1ALocalData(){
  try{
    if(localStorage.getItem('gold1g_migrated_from_gold1a')==='true') return;
    const oldPrefix='gold1a_', newPrefix='gold1g_';
    Object.keys(localStorage).forEach(k=>{
      if(k.startsWith(oldPrefix)){
        const nk=newPrefix+k.slice(oldPrefix.length);
        if(localStorage.getItem(nk)===null) localStorage.setItem(nk, localStorage.getItem(k));
    ...
    localStorage.setItem('gold1g_migrated_from_gold1a','true');
  }catch(e){console.warn('Gold 1G migration skipped', e)}
}
...
async function initFirebase(){try{fb=await import("./firebase.js");return fb}catch(e){console.warn("Gold 1G Firebase unavailable",e);return null}}
...
function defaultFiles(){return[
 {id:uid(),name:"Welcome to EmeraldOS Gold 1G.edoc",type:"doc",folder:"Documents",star:true,content:"<h1>Welcome to EmeraldOS Gold 1G</h1><p>This version improves setup, support, Staff Edition, Emerald BIOS, apps, logos, settings, and EmeraldOS Gold usability.</p>",created:now(),updated:now(),trash:false},
...
function mail(){return read(PREFIX+"mail",{inbox:[{id:uid(),from:"system@gold.mail",to:mailAddress(),subject:"Welcome to Gold Mail",body:"Gold Mail is ready in EmeraldOS Gold 1G.",read:false,time:now()}],sent:[],drafts:[],trash:[]})}function saveMail(m){write(PREFIX+"mail",m);renderNotifBadge();saveWorkspaceDebounced()}
function notes(){return read(PREFIX+"notes",[])}function saveNotes(n){write(PREFIX+"notes",n);saveWorkspaceDebounced()}
function tasks(){return read(PREFIX+"tasks",[{id:uid(),text:"Explore EmeraldOS Gold 1G",done:false,priority:"Normal",due:""}])}function saveTasks(t){write(PREFIX+"tasks",t);saveWorkspaceDebounced()}
...
APPS.push(
 {id:"help",name:"Get Help",label:"GH",color:"blue",group:"Support",desc:"Gold 1G help, tips, guides, and shortcuts.",open:openHelp},
...
function staffPreview(){notify("Staff login required","Open Staff Edition and sign in with a verified staff account.","Staff Edition");gold1gStaffGateNotice()}
...
function staffViewRemoteDesktop(sessionId){if(!isStaff()){gold1gStaffGateNotice();return}const w=openWindow('remoteview_'+sessionId,'Live Remote Desktop',remoteDesktopPreviewHTML(sessionId),{width:980,height:720,singleton:false});const int=setInterval(()=>{if(!document.body.contains(w)){clearInterval(int);return}const c=w.querySelector('.win-content');if(c)c.innerHTML=remoteDesktopPreviewHTML(sessionId)},1500)}
...
function openBIOSOptions(){openWindow("bios","BIOS Options",`<h2>Emerald BIOS A9</h2><div class="bios-embed"><p>Emerald BIOS options are available at startup using F12.</p><div class="toolbar"><button class="btn primary" onclick="location.href='bios.html'">Open full BIOS</button><button class="btn" onclick="localStorage.setItem('gold1g_safemode','true');location.reload()">Boot Safe Mode</button><button class="btn" onclick="localStorage.removeItem('gold1g_setup_done');location.reload()">Run Setup</button><button class="btn" onclick="location.href='staff.html'">Staff Edition</button></div></div>`,{width:720,height:420})}
...
function openSetup(){
  ...
  function setupHero(){return `<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>Welcome to EmeraldOS Gold 1G</h2><p class="muted">Set up your Gold virtual machine, personalize your desktop, choose your apps, and connect support tools.</p></div></div><div class="setup-checklist"><span>Account protected</span><span>Workspace continuity</span><span>Emerald support ready</span></div></div>`}
...
function finishSetup(){localStorage.setItem(PREFIX+'setup_done','true');$("setupWizard")?.classList.add('hidden');notify('Setup complete','EmeraldOS Gold 1G is ready.','Setup')}
...
function openNotepad(){openWindow('notepad','Notepad',`<h2>Notepad</h2><textarea id="np" class="field" rows="14">${esc(read(PREFIX+'notepad',''))}</textarea><button class="btn primary" onclick="localStorage.setItem('gold1g_notepad',JSON.stringify(np.value));Gold50.notify('Saved','Notepad saved','Notepad')">Save</button><button class="btn" onclick="Gold50.saveText('notepad.txt',np.value)">Export</button>`,{width:620,height:520})}
...
function openCalendar(){openWindow('calendar','Calendar',`<h2>Calendar</h2><p>${new Date().toDateString()}</p><textarea class="field" rows="8" onchange="localStorage.setItem('gold1g_calendar',JSON.stringify(this.value))">${esc(read(PREFIX+'calendar',''))}</textarea>`,{width:550,height:420})}
...
function openSecurity(){openWindow('security','Security Center',`<h2>Security Center</h2><div class="grid3"><div class="card"><h3>Safe Mode</h3><p>${localStorage.getItem(PREFIX+'safemode')==='true'?'Enabled':'Disabled'}</p><button class="btn" onclick="localStorage.setItem('gold1g_safemode','true');location.reload()">Enable</button><button class="btn" onclick="localStorage.removeItem('gold1g_safemode');location.reload()">Disable</button></div><div class="card"><h3>Blocked Users</h3><p>${read(PREFIX+'blocked',[]).length}</p></div><div class="card"><h3>Staff Session</h3><p>${isStaff()?'Active':'Not active'}</p></div></div>`,{width:760,height:480})}
...
function officeNew(type){
  ...
  if(type==='sheet')content=JSON.stringify([['Item','Amount','Notes'],['Apps','30','Gold 1G'],['Tickets','5','Support'],['Total','=SUM(B2:B3)','']]);
...
let saveTimer=null;function saveWorkspaceDebounced(){clearTimeout(saveTimer);saveTimer=setTimeout(()=>saveWorkspaceNow(false),700)}async function saveWorkspaceNow(show=true){const data={prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs(),savedAt:now(),build:BUILD.name};write(PREFIX+'workspace',data);if(fb?.setDoc&&fb?.doc&&fb?.db){try{await fb.setDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'goldVM','current'),data)}catch(e){console.warn('cloud save failed',e)}}if(show)notify('Workspace saved','Gold 1G workspace saved locally and attempted cloud sync.','Restore Center')}
async function restoreWorkspace(show=true){let data=read(PREFIX+'workspace',null);if(fb?.getDoc&&fb?.doc&&fb?.db){try{const snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'goldVM','current'));if(snap.exists())data=snap.data()}catch(e){console.warn('cloud restore failed',e)}}if(data){['prefs','files','tickets','mail','notes','tasks','contacts','notifications'].forEach(k=>{if(data[k])write(PREFIX+k,data[k])});applyPrefs();renderDesktop();renderNotifBadge();if(show)notify('Workspace restored','Gold 1G workspace restored.','Restore Center')}}function exportWorkspace(){saveText('emeraldos-gold-1g-workspace.json',JSON.stringify({prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs()},null,2),'application/json')}function importWorkspacePrompt(){const txt=prompt('Paste workspace JSON');if(!txt)return;try{const data=JSON.parse(txt);Object.keys(data).forEach(k=>write(PREFIX+k,data[k]));location.reload()}catch{alert('Invalid JSON')}}function resetGold(){if(confirm('Reset local EmeraldOS Gold 1G data?')){Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)).forEach(k=>localStorage.removeItem(k));location.reload()}}
function openApp(id){closeOverlays();const app=appById(id);if(app.staffOnly&&!isStaff()){gold1gStaffGateNotice();return}(app.open||openExplorer)()}function notifyPublic(t,b,a){return notify(t,b,a)}
...
========================================================= */
Object.assign(BUILD,{name:"EmeraldOS Gold 1G",version:"1G",cloudPath:"goldVM/current",bios:"Emerald BIOS A9"});
...
function applyPrefs(){const p=prefs();document.body.classList.toggle("dark",p.theme==="dark");document.body.classList.toggle("highcontrast",p.theme==="highcontrast");document.body.classList.toggle("bigtext",!!p.bigText);document.body.classList.toggle("no-motion",!!p.reducedMotion);document.body.classList.toggle("gold1g-solid-bg",!!p.useBackgroundColor);document.body.classList.toggle("gold1g-readable",!!p.readableFont);document.body.classList.toggle("gold1g-focus-ring",!!p.focusRings);document.body.classList.toggle("gold1g-large-cursor",!!p.largeCursor);document.body.classList.toggle("gold1g-underlined",!!p.underlineControls);document.body.classList.toggle("gold1g-extra-contrast",!!p.highContrastBorders);document.body.classList.toggle("gold1g-taskbar-top",p.taskbarPosition==="top");document.body.classList.toggle("gold1g-taskbar-compact",p.taskbarSize==="compact");document.body.classList.remove("bg-gold-bloom","bg-blue-window","bg-emerald-field","bg-slate-pro","custom-bg","gradient-aurora","gradient-skyline","gradient-emerald-pro","gradient-midnight","gradient-sunrise","gradient-ribbon","gradient-calm");if(p.customBackground){document.body.classList.add("custom-bg");document.body.style.setProperty("--customBg",`url(${p.customBackground})`);document.body.style.setProperty("--bgMode",p.bgMode||"cover");document.body.style.setProperty("--bgPosition",p.bgPosition||"center");document.body.style.setProperty("--bgRepeat",p.bgRepeat||"no-repeat")}else if(p.useBackgroundColor){document.body.style.setProperty("--gold1gBackground",p.backgroundColor||"#e9f3ff")}else{document.body.classList.add("gradient-"+(p.gradient||"aurora"));document.body.style.removeProperty("--customBg")}document.documentElement.style.setProperty("--accent",p.accent||"#0078d4");document.documentElement.style.setProperty("--accent2",p.secondaryAccent||"#0f7b4d");document.documentElement.style.setProperty("--gold70Secondary",p.secondaryAccent||"#0f7b4d");document.documentElement.style.setProperty("--winRadius",p.rounding==="none"?"0":p.rounding==="large"?"14px":p.rounding==="pill"?"20px":"6px");document.documentElement.style.setProperty("--gold1gTaskOpacity",String((Number(p.taskbarOpacity)||92)/100));document.documentElement.style.setProperty("--gold1gPanelOpacity",String((Number(p.panelOpacity)||94)/100));document.documentElement.style.setProperty("--gold1gMica",(Number(p.micaBlur)||0)+"px");document.documentElement.style.setProperty("--gold1gStartWidth",(Number(p.startWidth)||860)+"px");document.documentElement.style.setProperty("--gold1gTextScale",String(Number(p.textScale)||1));document.documentElement.style.setProperty("--gold1gBackground",p.backgroundColor||"#e9f3ff");document.documentElement.style.setProperty("--font",p.fontFamily||"Segoe UI");document.documentElement.style.setProperty("--gold1gIconScale",p.iconSize==="small"?".86":p.iconSize==="large"?"1.16":p.iconSize==="xl"?"1.32":"1");document.documentElement.style.setProperty("--gold1gDesktopGap",p.density==="compact"?"8px":p.density==="spacious"?"24px":"16px");const search=$("searchBtn"),widgets=$("widgetsBtn"),staff=$("staffBtn");if(search)search.style.display=p.taskbarSearch?"":"none";if(widgets)widgets.style.display=p.taskbarWidgets?"":"none";if(staff)staff.style.display=p.taskbarStaff?"":"none";}

function gold1gLocalState(){const localKeys={};Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)||["loggedIn","username","role","role2","userId"].includes(k)).forEach(k=>localKeys[k]=localStorage.getItem(k));return{build:BUILD.name,version:BUILD.version,savedAt:now(),user:username(),prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs(),userApps:read(PREFIX+'user_apps',[]),remoteSessions:read(PREFIX+'remote_sessions',[]),emergencyLogs:read(PREFIX+'emergency_logs',[]),snapshots:read(PREFIX+'vm_snapshots',[]),openWindows:[...document.querySelectorAll('.window')].map(w=>({id:w.id,title:w.dataset.title,left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,max:w.classList.contains('max'),min:w.classList.contains('minimized')})),localKeys};}
async function gold1gCloudWrite(data){if(!prefs().cloudSync)return false;if(!fb)await initFirebase();if(fb?.setDoc&&fb?.doc&&fb?.db){try{await fb.setDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'goldVM','current'),data);await fb.setDoc(fb.doc(fb.db,'emeraldOSGoldVMStates',username()),data);localStorage.setItem(PREFIX+'cloud_last_save',now());localStorage.setItem(PREFIX+'cloud_status','saved');return true}catch(e){console.warn('Gold 1G cloud save failed',e);localStorage.setItem(PREFIX+'cloud_status','save failed: '+(e.message||e));return false}}localStorage.setItem(PREFIX+'cloud_status','firebase unavailable');return false;}
async function gold1gCloudRead(){if(!fb)await initFirebase();if(fb?.getDoc&&fb?.doc&&fb?.db){try{let snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'goldVM','current'));if(snap.exists()){localStorage.setItem(PREFIX+'cloud_status','restored');return snap.data()}snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSGoldVMStates',username()));if(snap.exists()){localStorage.setItem(PREFIX+'cloud_status','restored');return snap.data()}}catch(e){console.warn('Gold 1G cloud restore failed',e);localStorage.setItem(PREFIX+'cloud_status','restore failed: '+(e.message||e))}}return null;}
...
async function saveWorkspaceNow(show=true){const data=gold1gLocalState();write(PREFIX+'workspace',data);const cloud=await gold1gCloudWrite(data);if(show)notify('Gold VM saved',cloud?'Everything was saved to the cloud VM state.':'Saved locally. Cloud sync will retry when Firebase is available.','Gold VM');return data;}
async function restoreWorkspace(show=true){let data=null;if(prefs().restoreOnLogin!==false)data=await gold1gCloudRead();if(!data)data=read(PREFIX+'workspace',null);if(data){if(data.localKeys)Object.entries(data.localKeys).forEach(([k,v])=>{if(v!==null&&v!==undefined)localStorage.setItem(k,String(v))});['prefs','files','tickets','mail','notes','tasks','contacts','notifications'].forEach(k=>{if(data[k])write(PREFIX+k,data[k])});if(data.userApps)write(PREFIX+'user_apps',data.userApps);if(data.remoteSessions)write(PREFIX+'remote_sessions',data.remoteSessions);if(data.emergencyLogs)write(PREFIX+'emergency_logs',data.emergencyLogs);if(data.snapshots)write(PREFIX+'vm_snapshots',data.snapshots);applyPrefs();renderDesktop();renderNotifBadge();if(show)notify('Gold VM restored','Your apps, files, settings, support history, and customizations were restored.','Gold VM')}else if(show)notify('No VM state found','Use setup or restore a backup to begin.','Gold VM')}
function exportWorkspace(){saveText('emeraldos-gold-8.0-cloud-vm-backup.json',JSON.stringify(gold1gLocalState(),null,2),'application/json')}
function resetGold(){if(confirm('Reset this EmeraldOS Gold 1G VM on this browser? Cloud data is not deleted.')){Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)).forEach(k=>localStorage.removeItem(k));location.reload()}}
function gold1gCloudStatusHTML(){return `<div class="grid4"><div class="card quick-card"><h3>Cloud sync</h3><p>${esc(localStorage.getItem(PREFIX+'cloud_status')||'ready')}</p></div><div class="card quick-card"><h3>Last save</h3><p>${esc(localStorage.getItem(PREFIX+'cloud_last_save')||'not yet')}</p></div><div class="card quick-card"><h3>VM path</h3><p>emeraldOSUsers/${esc(username())}/goldVM/current</p></div><div class="card quick-card"><h3>Tracked data</h3><p>apps, files, settings, tickets, mail, and workspace</p></div></div>`}
...
/* Gold 1G boot fix: these Gold 8 handlers are scoped inside seedGold50, so expose and install them when seedGold50 runs. */
  try{
    ...
    document.body.classList.add('gold1g','gold1g-shell');
    installGold80Apps();
    applyPrefs();
    if(!window.__gold1gAutoSaveInstalled){
      window.__gold1gAutoSaveInstalled=true;
      setInterval(()=>{const n=Number(prefs().autosaveInterval)||15;if(prefs().cloudSync!==false)saveWorkspaceNow(false)},15000);
    }
  }catch(e){console.error('Gold 1G handler install failed',e)}



/* EmeraldOS Gold 1G shell-managed update integration */
function gold1gShellManifest(){return read('emeraldGoldShell_latest',{latestVersion:'1G',folder:'Gold_1G',entry:'OS.html',releaseTitle:'EmeraldOS Gold 1G'});}
function gold1gVMState(){
  let cloudState={};
  try{cloudState=gold1gLocalState?gold1gLocalState():{};}catch(e){cloudState={};}
  return {
...
function gold1gPostState(reason='heartbeat'){
  try{
    if(window.parent&&window.parent!==window){
      window.parent.postMessage({type:'emeraldos-gold-live-state',reason,state:gold1gVMState()},'*');
    }
  }catch(e){console.warn('Gold 1G shell state post failed',e);}
}
function gold1gAskShellUpdateCheck(){try{window.parent?.postMessage({type:'emeraldos-gold-check-update',from:'Gold_1G',version:BUILD.version},'*');notify('Update check requested','The EmeraldOS Gold Shell is checking Firebase for the latest build.','Gold Update Center')}catch(e){notify('Update check failed',e.message,'Gold Update Center')}}
function openGold1AUpdateCenter(){
  const m=gold1gShellManifest();
  const active=localStorage.getItem('emeraldGoldShell_activeVersion')||BUILD.version;
  openWindow('updateshell','Gold Update Center',`<div class="goldshell-header"><div><h2>Gold Update Center</h2><p class="muted">Shell-managed updates keep your Gold VM data separate from version folders.</p></div><button class="btn primary" onclick="Gold50.gold1gAskShellUpdateCheck()">Check for updates</button></div><div class="grid2"><div class="card"><h3>Running build</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1G</p></div><div class="card"><h3>Latest Firebase pointer</h3><p>${esc(m.latestVersion||'Unknown')}</p><p class="muted">${esc(m.folder||'No folder')} / ${esc(m.entry||'OS.html')}</p></div><div class="card"><h3>Update behavior</h3><p>Gold Shell reads <code>system/emeraldGoldLatest</code>, saves a VM snapshot, then loads the folder Firebase points to.</p></div><div class="card"><h3>VM data</h3><p>Preferences, files, tickets, mail, apps, support history, and workspace state stay in the cloud VM profile.</p></div></div><h3>Release note</h3><p>${esc(m.summary||m.releaseTitle||'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.saveWorkspaceNow(true);Gold50.gold1gPostState('manual-save')">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html'">Return to Gold Shell</button></div>`,{width:980,height:680});
}
function installGold100ShellFeatures(){
  ...
  window.addEventListener('message',ev=>{const d=ev.data||{};if(d.type==='emeraldos-gold-latest'){write('emeraldGoldShell_latest',d.latest||{});notify('Update pointer refreshed',`Latest: ${(d.latest&&d.latest.latestVersion)||'unknown'}`,'Gold Update Center')}if(d.type==='emeraldos-gold-shell-save-request'){gold1gPostState('shell-save-request')}});
  window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{gold1gPostState('boot');setInterval(()=>gold1gPostState('heartbeat'),8000)},1700));
}
...
function applyGold50Visuals(){
  const p=prefs();
  document.body.classList.toggle('gold1g-solid-bg',!!p.useBackgroundColor);
  document.body.classList.toggle('gold1g-readable',!!p.readableFont);
  document.body.classList.toggle('gold1g-focus-ring',!!p.focusRing);
  document.body.classList.toggle('gold1g-large-cursor',!!p.largeCursor);
  document.body.classList.toggle('gold1g-links-underlined',!!p.underlineLinks);
  document.body.classList.toggle('gold1g-extra-contrast',!!p.extraContrast);
  document.body.classList.toggle('gold1g-less-blur',p.windowTransparency===false);
  document.body.classList.toggle('gold1g-compact-start',p.startStyle==='compact');
  document.body.classList.remove('gold1g-filter-none','gold1g-filter-warm','gold1g-filter-cool','gold1g-filter-gray');
  document.body.classList.add('gold1g-filter-'+(p.colorFilter||'none'));
  document.documentElement.style.setProperty('--gold1gBackgroundColor',p.backgroundColor||'#edf2f8');
  document.documentElement.style.setProperty('--gold1gTextScale',((Number(p.textScale)||100)/100).toString());
  document.documentElement.style.setProperty('--gold1gTaskbarHeight',p.taskbarSize==='large'?'56px':p.taskbarSize==='compact'?'40px':'48px');
}
...
function settings50Page(tab){const p=prefs();
 ...
 if(tab==='update')return `<h2>Update & Security</h2><div class="grid2"><div class="card"><h3>EmeraldOS Gold 1G</h3><p>Development, accessibility, personalization, and EmeraldOS Gold user-experience update.</p></div><div class="card"><h3>Safe Mode</h3><button class="btn" onclick="Gold50.openApp('safemode')">Open Safe Mode</button></div></div>`;
...
function getGold50Packages(){return gold1gUserApps()}
...
window.addEventListener('message',e=>{const d=e.data||{};if(d.source!=='gold1g-package')return;if(d.type==='notify')notify(d.title||'Package App',d.message||'',d.app||'Package App');if(d.type==='export')saveText(d.name||'package-export.txt',d.text||'');if(d.type==='open')openApp(d.id||'settings')});
...
function applyGold70Visuals(){
  const p=prefs();
  document.body.classList.add('gold1g');
  ...
  document.documentElement.style.setProperty('--gold1gSecondary',p.secondaryAccent||'#0f7b4d');
  document.documentElement.style.setProperty('--gold1gTaskOpacity',String((Number(p.taskbarOpacity)||92)/100));
  document.documentElement.style.setProperty('--gold1gStartWidth',(Number(p.startWidth)||860)+'px');
  document.documentElement.style.setProperty('--gold1gIconScale',p.iconSize==='large'?'1.14':p.iconSize==='small'?'.88':'1');
  document.documentElement.style.setProperty('--gold1gDesktopGap',p.density==='compact'?'9px':p.density==='spacious'?'24px':'16px');
  document.documentElement.style.setProperty('--gold1gMica',(Number(p.micaBlur)||18)+'px');
}
...
function settings60Page(tab){const p=prefs();
  ...
  if(tab==='privacy')return `<h2>Privacy</h2><label class="setting-line"><span>Focus Assist</span><input type="checkbox" ${p.focusAssist?'checked':''} onchange="Gold50.setGold70Pref('focusAssist',this.checked)"></label><label class="setting-line"><span>Notifications</span><input type="checkbox" ${p.notifications!==false?'checked':''} onchange="Gold50.setGold70Pref('notifications',this.checked)"></label><label class="setting-line"><span>Remote assistance requires visible user consent</span><input type="checkbox" checked disabled></label><button class="btn" onclick="localStorage.removeItem('${PREFIX}search_history');Gold50.notify('Privacy','Search history cleared','Settings')">Clear local search history</button>`;
  if(tab==='update')return `<h2>Update & Security</h2><div class="grid2"><div class="card"><h3>EmeraldOS Gold 1G</h3><p>Personalization, Staff resources, Support, BIOS A9, and EmeraldOS Gold-style user friendliness.</p></div><div class="card"><h3>Safe Mode</h3><button class="btn" onclick="Gold50.openApp('safemode')">Open Safe Mode</button></div></div>`;
...
function openStaffCenter60(){if(!isStaff()){gold1gStaffGateNotice();return}openWindow('staff','Gold Staff Center',staffDashboard60(),{width:1120,height:760})}
...
function openStaffKnowledge60(){if(!isStaff()){gold1gStaffGateNotice();return}const q='';openWindow('staffkb','Staff Knowledge Base',staffKnowledgeHTML(q),{width:980,height:700});window.Gold50.searchStaffKB60=(v)=>{const b=document.querySelector('#win_staffkb .win-content');if(b)b.innerHTML=staffKnowledgeHTML(v)};}
...
function openStaffReports60(){if(!isStaff()){gold1gStaffGateNotice();return}const open=tickets().filter(t=>t.status!=='Resolved').length,res=tickets().filter(t=>t.status==='Resolved').length,remote=remoteSessions().length;openWindow('staffreports','Staff Reports',`<h2>Staff Reports</h2><div class="grid4"><div class="card"><h3>Open tickets</h3><p>${open}</p></div><div class="card"><h3>Resolved</h3><p>${res}</p></div><div class="card"><h3>Remote sessions</h3><p>${remote}</p></div><div class="card"><h3>Staff logs</h3><p>${read(PREFIX+'staff_logs',[]).length}</p></div></div><div class="toolbar"><button class="btn primary" onclick="Gold50.saveText('gold1g-tickets.json',JSON.stringify(Gold50.tickets?Gold50.tickets():[],null,2),'application/json')">Export Tickets</button><button class="btn" onclick="Gold50.staffLogs()">Open Staff Logs</button></div><h3>Recent logs</h3>${read(PREFIX+'staff_logs',[]).slice(0,8).map(l=>`<div class="card"><b>${esc(l.type)}</b><p>${esc(l.note)}</p><small>${new Date(l.time).toLocaleString()}</small></div>`).join('')||'<p class="muted">No logs.</p>'}`,{width:920,height:680})}
function openStaffBroadcast60(){if(!isStaff()){gold1gStaffGateNotice();return}openWindow('staffbroadcast','Staff Broadcast',`<h2>Staff Broadcast</h2><p class="muted">Send a visible local notification. Future versions can sync broadcasts through Firestore.</p><label>Title<input id="bc_title" class="field" value="Staff Broadcast"></label><label>Message<textarea id="bc_body" class="field" rows="6"></textarea></label><button class="btn primary" onclick="Gold50.sendStaffBroadcast60()">Send Broadcast</button>`,{width:700,height:520})}
...
function applyGold70SupportVisuals(){document.body.classList.add('gold1g-goldshell');const p=prefs();document.documentElement.style.setProperty('--gold1gTileOpacity',p.tileOpacity||'.94');document.documentElement.style.setProperty('--gold1gPanelRadius',p.goldshellRadius||'8px')}
...
/* EmeraldOS Gold 1G rebuilt update-system fallback.
   Some Gold 1G update helpers are created inside the seeded boot scope.
...
function gold1gRebuiltSnapshot(){
  return {
    product:'EmeraldOS Gold',
    version:BUILD.version,
    folder:'Gold_1G',
...
function gold1gRebuiltLocalManifest(){
  return read('emeraldGoldShell_latest',{latestVersion:'1G',build:'1G',folder:'Gold_1G',entry:'OS.html',releaseTitle:'EmeraldOS Gold 1G',summary:'EmeraldOS Gold update-system build.'});
}
async function gold1gRebuiltFetchLatest(){
  try{
    const fbm = await import('./firebase.js');
    const snap = await fbm.getDoc(fbm.doc(fbm.db,'system','emeraldGoldLatest'));
    const latest = snap.exists()?Object.assign(gold1gRebuiltLocalManifest(),snap.data()):gold1gRebuiltLocalManifest();
    write('emeraldGoldShell_latest',latest);
    return latest;
  }catch(e){
    console.warn('Gold 1G update check Firebase unavailable',e);
    return gold1gRebuiltLocalManifest();
  }
}
function gold1gRebuiltSaveBeforeUpdate(){
  try{saveWorkspaceNow(false)}catch(e){console.warn(e)}
  try{write('emeraldGoldShell_liveState',gold1gRebuiltSnapshot())}catch(e){console.warn(e)}
}
async function gold1gRebuiltCheckForUpdates(){
  const latest = await gold1gRebuiltFetchLatest();
  const current = BUILD.version;
  const folder = latest.folder || 'Gold_1G';
  const version = latest.latestVersion || latest.build || '1B';
  const message = version===current && folder==='Gold_1G'
...
function gold1gRebuiltApplyUpdate(){
  gold1gRebuiltSaveBeforeUpdate();
  localStorage.setItem('emeraldGoldShell_forceCheck','true');
  localStorage.setItem('emeraldGoldShell_returnedFromVersion','Gold_1G');
  location.href='../gold-shell.html?force=1';
}
function openGold1AUpdateCenterRebuilt(manifest){
  const m=manifest||gold1gRebuiltLocalManifest();
  const version=m.latestVersion||m.build||'1B';
  const folder=m.folder||'Gold_1G';
  const entry=m.entry||'OS.html';
  const currentFolder='Gold_1G';
  const isDifferent=(version!==BUILD.version)||(folder!==currentFolder);
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">EmeraldOS Gold uses a root updater that checks Firebase, saves the VM, and boots the configured version folder. The updater is not visible after boot.</p></div><button class="btn primary" onclick="Gold50.gold1gCheckForUpdates()">Check now</button></div><div class="grid2"><div class="card"><h3>Current VM</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: ${currentFolder}</p></div><div class="card"><h3>Configured latest</h3><p>${esc(version)}</p><p class="muted">${esc(folder)} / ${esc(entry)}</p></div><div class="card"><h3>Status</h3><p>${isDifferent?'An update or folder change is available.':'No newer folder is currently configured.'}</p></div><div class="card"><h3>VM continuity</h3><p>Before applying an update, Gold saves a workspace snapshot so your files, settings, apps, tickets, and desktop state can continue.</p></div></div><h3>Release note</h3><p>${esc(m.summary||m.releaseTitle||'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.gold1gSaveUpdateSnapshot()">Save VM snapshot</button><button class="btn primary" onclick="Gold50.gold1gApplyUpdate()">Restart through System Update</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button></div>`,{width:980,height:680});
}
Object.assign(window.Gold50,{
  openGold1AUpdateCenter:(typeof openGold1AUpdateCenter==='function'?openGold1AUpdateCenter:openGold1AUpdateCenterRebuilt),
  gold1gAskShellUpdateCheck:(typeof gold1gAskShellUpdateCheck==='function'?gold1gAskShellUpdateCheck:gold1gRebuiltCheckForUpdates),
  gold1gCheckForUpdates:gold1gRebuiltCheckForUpdates,
  gold1gApplyUpdate:gold1gRebuiltApplyUpdate,
  gold1gSaveUpdateSnapshot:()=>{gold1gRebuiltSaveBeforeUpdate();notify('VM snapshot saved','EmeraldOS Gold saved a local update snapshot.','System Update')},
  gold1gPostState:(typeof gold1gPostState==='function'?gold1gPostState:()=>write('emeraldGoldShell_liveState',gold1gRebuiltSnapshot())),
  gold1gVMState:(typeof gold1gVMState==='function'?gold1gVMState:gold1gRebuiltSnapshot),
...
try{const updateApp=APPS.find(a=>a.id==='updateshell');const updateDef={id:'updateshell',name:'System Update',label:'UP',color:'blue',group:'System',desc:'Check Firebase for the latest EmeraldOS Gold folder and restart through the updater.',open:window.Gold50.openGold1AUpdateCenter};if(updateApp)Object.assign(updateApp,updateDef);else APPS.push(updateDef);}catch(e){console.warn('Gold 1G update app install failed',e)}
try{enhanceSettings70()}catch(e){console.warn('Gold 1G settings enhancer failed',e)}


/* Gold 1G boot fix: final exposure is installed inside seedGold50 so boot never references scoped handlers too early. */
function gold1gInstallLiveHandlers(){return true;}
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{if(window.Gold50?.openCloudSyncCenter80){document.body.classList.add('gold1g','gold1g-shell');}}catch(e){console.warn(e)}},120));


/* EmeraldOS Gold 1G first-run Firestore rules installer.
...
function gold1gRulesText(){return `rules_version = '2';
...
`;}
function gold1gRulesStatus(){return read(PREFIX+'rules_status',{ran:false,status:'not_run',message:'Rules installer has not run yet.'});}
function gold1gWriteRulesStatus(status){write(PREFIX+'rules_status',status);return status;}
async function gold1gStageFirestoreRules(){
  const status={ran:true,status:'staging',version:'1G',time:now(),message:'Preparing EmeraldOS Gold 1G rules.'};
  gold1gWriteRulesStatus(status);
  let fbm=null;
  try{fbm=await import('./firebase.js');}catch(e){status.status='local_only';status.message='Firebase module unavailable. Rules saved locally only.';status.error=e.message;gold1gWriteRulesStatus(status);return status;}
  const payload={product:'EmeraldOS Gold',version:'1G',rulesVersion:'gold10-default',rules:gold1gRulesText(),createdBy:username(),createdAt:now(),status:'pending_deployment',note:'Generated automatically by EmeraldOS Gold 1G first boot. Deploy from Firebase Console/CLI or a secured deployer endpoint.'};
  try{
...
async function gold1gTrySecureRulesDeploy(){
  ...
  const body={product:'EmeraldOS Gold',version:'1G',rules:gold1gRulesText(),requestedBy:username(),time:now()};
...
async function gold1gAutoRulesInstall(silent=false){
  if(localStorage.getItem(PREFIX+'rules_install_ran')==='true') return gold1gRulesStatus();
  localStorage.setItem(PREFIX+'rules_install_ran','true');
  const staged=await gold1gStageFirestoreRules();
  try{
    const deployed=await gold1gTrySecureRulesDeploy();
    if(deployed && (deployed.ok||deployed.deployed)){
      staged.status='deployed';staged.message='Rules were deployed through the configured secure deployer.';staged.deployerResult=deployed;gold1gWriteRulesStatus(staged);
    }
  }catch(e){staged.deployerError=e.message;gold1gWriteRulesStatus(staged);}
  if(!silent){notify('Rules installer finished',gold1gRulesStatus().message,'Security Rules');}
  return gold1gRulesStatus();
}
function openGoldRulesInstaller100(){
  const s=gold1gRulesStatus();
  openWindow('rulesinstaller','Security Rules Installer',`<div class="goldshell-header"><div><h2>Security Rules Installer</h2><p class="muted">Gold 1G prepares the recommended Firestore rules on first boot. Direct deployment requires Firebase Console/CLI or a secure deployer endpoint.</p></div><button class="btn primary" onclick="Gold50.gold1gAutoRulesInstall(false)">Run installer</button></div><div class="grid2"><div class="card"><h3>Status</h3><p>${esc(s.status)}</p><p class="muted">${esc(s.message||'')}</p></div><div class="card"><h3>Secure deployer</h3><p>Optional. Set <code>emeraldGoldRulesDeployerUrl</code> and <code>emeraldGoldRulesDeployerKey</code> in localStorage for automatic deployment through your own backend.</p></div></div><h3>Rules</h3><textarea class="field mono" rows="18">${esc(gold1gRulesText())}</textarea><div class="toolbar"><button class="btn" onclick="navigator.clipboard?.writeText(Gold50.gold1gRulesText())">Copy rules</button><button class="btn" onclick="location.href='rules-installer.html'">Open full rules page</button></div>`,{width:1080,height:760});
}
function installGold100RulesInstaller(){
  try{
    const old=APPS.find(a=>a.id==='rulesinstaller');
    const app={id:'rulesinstaller',name:'Security Rules Installer',label:'RI',color:'red',group:'System',desc:'Stages Gold 1G Firestore rules and deploys through an optional secure backend.',staffOnly:true,open:openGoldRulesInstaller100};
  ...
  }catch(e){console.warn('Gold 1G rules app install failed',e)}
  /* Rules installer is manual in redesigned Gold 1G. */
}
installGold100RulesInstaller();
Object.assign(window.Gold50,{openGoldRulesInstaller100,gold1gAutoRulesInstall,gold1gRulesText,gold1gRulesStatus,gold1gStageFirestoreRules,gold1gTrySecureRulesDeploy});



/* EmeraldOS Gold 1G redesigned manual update architecture.
   The root shell checks Firebase, but each user chooses when to move their VM to a newer folder.
   Gold_1G can publish its own latest pointer when manually booted once by staff. */
const GOLD1A_LATEST_MANIFEST = {
  ...
  enabled: true,
  setupMode: "continue",
  releaseTitle: "EmeraldOS Gold 1G",
...
function gold1gCurrentManifest(){ return {...GOLD1A_LATEST_MANIFEST}; }
function gold1gLatestCache(){ return read('emeraldGoldShell_latest', GOLD1A_LATEST_MANIFEST); }
function gold1gIsNewerManifest(latest){
  if(!latest) return false;
  const latestVersion = String(latest.latestVersion || latest.build || '');
  const latestFolder = String(latest.folder || '');
  return latestVersion !== BUILD.version || latestFolder !== 'Gold_1G';
}
async function gold1gFetchLatestManifest(){
  try{
    const fbm = await import('./firebase.js');
    const snap = await fbm.getDoc(fbm.doc(fbm.db, 'system', 'emeraldGoldLatest'));
    ...
    console.warn('Gold 1G could not read update manifest', error);
    return gold1gLatestCache();
  }
}
function gold1gUpdateSnapshot(){
  const data = {
    product:'EmeraldOS Gold',
    version:BUILD.version,
    folder:'Gold_1G',
...
async function gold1gCloudWrite(path, value){
  try{ const fbm = await import('./firebase.js'); await fbm.setDoc(fbm.doc(fbm.db, ...path), value, {merge:true}); return true; }
  catch(error){ console.warn('Gold 1G cloud write failed', path, error); return false; }
}
async function gold1gSaveManualUpdateSnapshot(){
  const snap = gold1gUpdateSnapshot();
  const user = username();
  await gold1gCloudWrite(['emeraldOSUsers', user, 'goldVM', 'current'], {
    activeVersion: BUILD.version,
    activeFolder: 'Gold_1G',
    entry: 'OS.html',
    lastSave: now(),
    cloudSync: true
  });
  await gold1gCloudWrite(['emeraldOSUsers', user, 'goldVMSnapshots', `manual_update_${Date.now()}`], snap);
  notify('VM snapshot saved','Your EmeraldOS Gold VM state was saved before update.','System Update');
  return snap;
}
async function gold1gApplyManualUpdate(){
  const latest = await gold1gFetchLatestManifest();
  if(!latest || !latest.folder){ alert('No update target is configured.'); return; }
  await gold1gSaveManualUpdateSnapshot();
...
function gold1gSkipUpdate(version){
...
function gold1gRemindLater(version){
...
function gold1gUpdatePromptHTML(latest){
  const version = esc(latest.latestVersion || latest.build || 'Unknown');
  const folder = esc(latest.folder || '');
  return `<div class="update-prompt"><h2>EmeraldOS Gold update available</h2><p>A newer EmeraldOS Gold version is available for your cloud VM.</p><div class="grid2"><div class="card"><h3>Current</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1G</p></div><div class="card"><h3>Available</h3><p>EmeraldOS Gold ${version}</p><p class="muted">Folder: ${folder}</p></div></div><h3>Release note</h3><p>${esc(latest.summary || latest.releaseTitle || 'No release note was provided.')}</p><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1gApplyManualUpdate()">Update this VM now</button><button class="btn" onclick="Gold50.gold1gRemindLater('${version}')">Remind me later</button><button class="btn" onclick="Gold50.gold1gSkipUpdate('${version}')">Skip this version</button><button class="btn" onclick="Gold50.openGold1AUpdateCenter()">Open System Update</button></div><p class="muted">Updates are manual per user. Your VM keeps running this version until you choose to update.</p></div>`;
}
function gold1gOpenUpdatePrompt(latest){
  openWindow('updateprompt','EmeraldOS Gold Update Available',gold1gUpdatePromptHTML(latest),{width:860,height:590});
}
async function gold1gCheckLoginUpdateNotice(){
  const latest = await gold1gFetchLatestManifest();
  const latestVersion = String(latest.latestVersion || latest.build || '');
  ...
  const newer = gold1gIsNewerManifest(latest);
  write('emeraldGold_updateAvailable', {available:newer, latest, active:gold1gCurrentManifest(), checkedAt:now()});
  if(newer && skipped !== latestVersion && !(laterVersion === latestVersion && Date.now() < laterUntil)){
    notify('EmeraldOS Gold update available',`Version ${latestVersion} is ready. You can update now or stay on Gold 1G.`, 'System Update');
    setTimeout(()=>gold1gOpenUpdatePrompt(latest), 700);
  }
}
function openGold1AUpdateCenter(latestManifest){
  const latest = latestManifest || gold1gLatestCache();
  const newer = gold1gIsNewerManifest(latest);
...
function gold1gInstallManualUpdateSystem(){
  try{
    ...
    if(old) Object.assign(old, app); else APPS.push(app);
  }catch(error){ console.warn('Gold 1G manual update app install failed', error); }
}
gold1gInstallManualUpdateSystem();
Object.assign(window.Gold50, {
  openGold1AUpdateCenter,
  gold1gCheckForUpdatesManual,
  gold1gApplyManualUpdate,
  gold1gSaveManualUpdateSnapshot,
  gold1gResetUpdateChoice,
  gold1gSkipUpdate,
  gold1gRemindLater,
  gold1gPublishLatestManifest,
  gold1gFetchLatestManifest,
...
/* EmeraldOS Gold 1G OS-folder-only update helpers */
function gold1gLatestManifest(){
  return {
    product:'EmeraldOS Gold', latestVersion:'1G', build:'1G', folder:'Gold_1G', entry:'OS.html',
...
async function gold1gPublishLatest(showResult=true){
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
    const {doc,setDoc}=api;
    const manifest=gold1gLatestManifest();
    await setDoc(doc(api.db,'system','emeraldGoldLatest'), manifest, {merge:true});
    localStorage.setItem('gold1g_published_latest','true');
    if(showResult) notify('System Update','Gold 1G was published as the latest available build.','System Update');
    return true;
  }catch(e){
    console.warn('Gold 1G publish latest failed', e);
    if(showResult) notify('System Update','Could not publish latest build. Check Firebase rules/access.','System Update');
    return false;
  }
}
function openGold1AUpdateCenter(){
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">Gold 1G is an OS folder only. The shell stays outside this folder and only routes updates when users choose to update.</p></div><button class="btn primary" onclick="Gold50.gold1gCheckForUpdatesManual?.()||Gold50.gold1gCheckForUpdates?.()">Check Firebase</button></div><div class="grid2"><div class="card"><h3>This folder</h3><p>EmeraldOS Gold 1G</p><p class="muted">Gold_1G / OS.html</p></div><div class="card"><h3>Publish flow</h3><p>Only Staff Edition users can publish this version. Open Update Publisher Manager, enter the publisher PIN, then click Publish this Version to set <code>system/emeraldGoldLatest</code>.</p></div><div class="card"><h3>User updates</h3><p>Each user updates manually. The shell notifies them on login and only changes their active folder after they accept.</p></div><div class="card"><h3>Continuity</h3><p>VM data remains stored under <code>goldVM/current</code>, independent of the version folder.</p></div></div><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1gPublishLatest(true)">Publish Gold 1G as latest</button><button class="btn" onclick="Gold50.saveWorkspaceNow(true)">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html?applyUpdate=1'">Restart through shell</button></div>`,{width:1040,height:700});
}
function gold1gInstallUpdateApp(){
  try{
    const app={id:'updateshell',name:'System Update',label:'UP',color:'blue',group:'System',desc:'Check updates, save VM state, and restart through the external EmeraldOS Gold shell. Publishing is Staff Edition only.',open:openGold1AUpdateCenter};
    const existing=APPS.find(a=>a.id==='updateshell');
    if(existing) Object.assign(existing,app); else APPS.push(app);
  }catch(e){console.warn('Gold 1G update app install failed',e)}
}
gold1gInstallUpdateApp();
Object.assign(window.Gold50||{}, {gold1gLatestManifest,gold1gPublishLatest,openGold1AUpdateCenter});
window.Gold1A=window.Gold50; window.Gold1G=window.Gold50;
// Query-string publishing disabled in Gold 1G. Use Staff Edition > Update Publisher Manager > PIN > Publish this Version.


/* =========================================================
   EMERALDOS GOLD 1G - E.L.S.U.S. COMPATIBILITY + UX PATCH
...
const GOLD1A_RELEASE_MANIFEST={
  ...
  channel:'stable', status:'stable', required:false, enabled:true, setupMode:'continue',
  releaseTitle:'EmeraldOS Gold 1G',
  summary:'Gold 1G is an OS-folder-only E.L.S.U.S.-compatible release built from Gold 11.0. It improves the EmeraldOS Gold desktop experience, adds a curated non-duplicate app catalog, improves customization safety, and keeps VM data version-independent.',
  migrationFrom:['11.0','10.0','9.0','8.0','8.0.1'], migrationId:'gold11-to-gold1g-elsus',
  minShellVersion:'1.0', rollbackFolder:'Gold_11.0', rollbackVersion:'11.0', releasedAt:new Date().toISOString()
};
function gold1gClamp(n,min,max,fall){n=Number(n);if(!Number.isFinite(n))n=fall;return Math.max(min,Math.min(max,n));}
function gold1gCSSColor(v,fall){v=String(v||'').trim();return /^#[0-9a-f]{3,8}$/i.test(v)?v:fall;}
function gold1gCurrentManifest(){return {...GOLD1A_RELEASE_MANIFEST};}
async function gold1gPublishLatest(showResult=true){
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
...
function gold1gInstallCuratedApps(){
  ...
  unique.forEach(gold1gAddOrUpdateApp);
  ...
  pref.backgroundColor=pref.backgroundColor||'#eaf3ff'; pref.panelOpacity=gold1gClamp(pref.panelOpacity,70,100,94); pref.taskbarOpacity=gold1gClamp(pref.taskbarOpacity,60,100,92);
  pref.startWidth=gold1gClamp(pref.startWidth,560,1100,880); pref.micaBlur=gold1gClamp(pref.micaBlur,0,30,18);
  write(PREFIX+'prefs',pref);
}
function gold1gApplyCustomizationSafeCss(){
  const p=prefs(); const root=document.documentElement;
  document.body.classList.add('gold1g','gold1g-elsus');
  document.body.classList.toggle('gold1g-solid-bg',!!p.useBackgroundColor && !p.customBackground);
  document.body.classList.toggle('gold1g-taskbar-top',p.taskbarPosition==='top');
  root.style.setProperty('--gold1gBgColor',gold1gCSSColor(p.backgroundColor,'#eaf3ff'));
  root.style.setProperty('--gold1gAccent',gold1gCSSColor(p.accent,'#0078d4'));
  root.style.setProperty('--gold1gAccent2',gold1gCSSColor(p.secondaryAccent,'#0f7b4d'));
  root.style.setProperty('--accent',gold1gCSSColor(p.accent,'#0078d4'));
  root.style.setProperty('--accent2',gold1gCSSColor(p.secondaryAccent,'#0f7b4d'));
  root.style.setProperty('--gold1gPanelOpacity',String(gold1gClamp(p.panelOpacity,70,100,94)/100));
  root.style.setProperty('--gold1gTaskOpacity',String(gold1gClamp(p.taskbarOpacity,60,100,92)/100));
  root.style.setProperty('--gold1gMica',gold1gClamp(p.micaBlur,0,30,18)+'px');
  root.style.setProperty('--gold1gStartWidth',gold1gClamp(p.startWidth,560,1100,880)+'px');
  root.style.setProperty('--gold1gIconScale',p.iconSize==='xl'?'1.28':p.iconSize==='large'?'1.14':p.iconSize==='small'?'.88':'1');
  root.style.setProperty('--gold1gDesktopGap',p.density==='compact'?'9px':p.density==='spacious'?'24px':'16px');
  root.style.setProperty('--winRadius',p.rounding==='none'?'0':p.rounding==='large'?'14px':p.rounding==='pill'?'18px':'8px');
}
function gold1gWireCustomizationSafety(){
  const oldSetGold70=window.Gold50?.setGold70Pref;
  if(oldSetGold70&&!oldSetGold70.gold1gWrapped){
    const wrapped=function(k,v){oldSetGold70(k,v);gold1gApplyCustomizationSafeCss();try{renderDesktop()}catch{}};wrapped.gold1gWrapped=true;window.Gold50.setGold70Pref=wrapped;
  }
  const oldSetPrefs=window.Gold50?.setPrefs;
  if(oldSetPrefs&&!oldSetPrefs.gold1gWrapped){
    const wrapped=function(p){oldSetPrefs(p);gold1gApplyCustomizationSafeCss();try{renderDesktop()}catch{}};wrapped.gold1gWrapped=true;window.Gold50.setPrefs=wrapped;
  }
}
function openGold1AWelcome(){
  openWindow('emeraldbrand','EmeraldOS Gold 1G',`<div class="welcome70"><div class="gold1g-hero"><div class="gold-logo-xl">G</div><div><h1>EmeraldOS Gold 1G</h1><p>OS-folder-only E.L.S.U.S. release with a cleaner EmeraldOS Gold desktop, curated app catalog, safer customization, and cloud VM continuity.</p></div></div><div class="grid3"><div class="card"><h3>Customize everything</h3><p>Theme Studio controls background color, wallpaper, accents, taskbar, panels, blur, rounding, desktop spacing, and accessibility.</p><button class="btn primary" onclick="Gold50.openApp('themestudio')">Open Theme Studio</button></div><div class="card"><h3>Cloud VM</h3><p>Your files, settings, apps, tickets, mail, notes, tasks, and workspace stay separate from the version folder.</p><button class="btn" onclick="Gold50.openApp('cloudsync')">Open Cloud Sync</button></div><div class="card"><h3>E.L.S.U.S.</h3><p>The updater shell prompts users and only changes their active version when they choose to update.</p><button class="btn" onclick="Gold50.openApp('updateshell')">System Update</button></div></div></div>`,{width:940,height:620});
}
function gold1gInstallBootPolish(){
  gold1gInstallCuratedApps();
  gold1gApplyCustomizationSafeCss();
  gold1gWireCustomizationSafety();
  gold1gAddOrUpdateApp({id:'emeraldbrand',name:'EmeraldOS Gold Welcome',label:'EG',color:'gold',group:'Core',purpose:'Welcome',desc:'Overview of Gold 1G, E.L.S.U.S., customization, and cloud VM continuity.',open:openGold1AWelcome});
...
window.Gold1A=window.Gold50; window.Gold1G=window.Gold50;
window.addEventListener('DOMContentLoaded',()=>setTimeout(gold1gInstallBootPolish,120));
// Query-string publisher opening disabled in Gold 1G.


/* =========================================================
   EMERALDOS GOLD 1G UPDATE SETUP + CLASSIC RIGHT-CLICK MENU
========================================================= */
const GOLD1G_ESSENTIAL_DESKTOP=['explorer','office','mail','support','settings','updateshell'];
function gold1gDesktopPins(){return read(PREFIX+'desktopFilePins',[])}
function gold1gSaveDesktopPins(arr){write(PREFIX+'desktopFilePins',[...new Set(arr)].filter(Boolean));saveWorkspaceDebounced();try{renderDesktop()}catch{}}
function gold1gStartPins(){return read(PREFIX+'startPins',['explorer','office','support','settings','updateshell'])}
function gold1gTaskbarPins(){return read(PREFIX+'taskbarPins',['explorer','office','support'])}
function gold1gSetStartPins(arr){write(PREFIX+'startPins',[...new Set(arr)].filter(Boolean));try{renderStartMenu()}catch{};saveWorkspaceDebounced()}
function gold1gSetTaskbarPins(arr){write(PREFIX+'taskbarPins',[...new Set(arr)].filter(Boolean));try{renderTaskApps()}catch{};saveWorkspaceDebounced()}
function gold1gPin(arr,id){return [...new Set([...(arr||[]),id])].filter(Boolean)}
function gold1gUnpin(arr,id){return (arr||[]).filter(x=>x!==id)}
function gold1gApplyEssentialDesktop(){const p=prefs();p.desktopApps=[...GOLD1G_ESSENTIAL_DESKTOP];write(PREFIX+'prefs',p);applyPrefs();renderDesktop();renderStartMenu();notify('Desktop cleaned','Only essential apps are pinned to the desktop. All apps are still in Start and Search.','Setup')}
function gold1gPinAppToDesktop(id){const p=prefs();p.desktopApps=gold1gPin(p.desktopApps||[],id);write(PREFIX+'prefs',p);applyPrefs();renderDesktop();notify('Pinned to desktop',appById(id)?.name||id,'Desktop')}
function gold1gRemoveAppFromDesktop(id){const p=prefs();p.desktopApps=gold1gUnpin(p.desktopApps||[],id);write(PREFIX+'prefs',p);applyPrefs();renderDesktop();notify('Removed from desktop',appById(id)?.name||id,'Desktop')}
function gold1gPinAppToStart(id){gold1gSetStartPins(gold1gPin(gold1gStartPins(),id));notify('Pinned to Start',appById(id)?.name||id,'Start')}
function gold1gUnpinAppFromStart(id){gold1gSetStartPins(gold1gUnpin(gold1gStartPins(),id));notify('Unpinned from Start',appById(id)?.name||id,'Start')}
function gold1gPinAppToTaskbar(id){gold1gSetTaskbarPins(gold1gPin(gold1gTaskbarPins(),id));notify('Pinned to taskbar',appById(id)?.name||id,'Taskbar')}
function gold1gUnpinAppFromTaskbar(id){gold1gSetTaskbarPins(gold1gUnpin(gold1gTaskbarPins(),id));notify('Unpinned from taskbar',appById(id)?.name||id,'Taskbar')}
function gold1gPinFileToDesktop(fid){if(!files().some(f=>f.id===fid)){notify('File not found','That file could not be pinned.','Desktop');return}gold1gSaveDesktopPins(gold1gPin(gold1gDesktopPins(),fid));notify('Pinned file to desktop',files().find(f=>f.id===fid)?.name||fid,'Gold Explorer')}
function gold1gUnpinFileFromDesktop(fid){gold1gSaveDesktopPins(gold1gUnpin(gold1gDesktopPins(),fid));notify('Unpinned file from desktop',files().find(f=>f.id===fid)?.name||fid,'Desktop')}
function gold1gFileDesktopIcon(f){return `<button class="desktop-icon desktop-file ${prefs().showLabels?'':'no-label'}" data-fileid="${esc(f.id)}" ondblclick="Gold50.openFile('${esc(f.id)}')" title="${esc(f.name)}"><div class="file-desktop-icon">${esc(fileIcon(f.type))}</div><span>${esc(f.name)}</span></button>`}
try{
  const gold1gOriginalRenderDesktop=renderDesktop;
  renderDesktop=function(){
    ...
    const pinIds=gold1gDesktopPins();
    const fileHTML=pinIds.map(fid=>files().find(f=>f.id===fid&&!f.trash)).filter(Boolean).map(gold1gFileDesktopIcon).join('');
    d.innerHTML=appHTML+fileHTML;
  };
}catch(e){console.warn('Gold 1G desktop override skipped',e)}
try{
  const gold1gOriginalRenderTaskApps=renderTaskApps;
  renderTaskApps=function(){
    const bar=$('taskApps'); if(!bar) return;
    const pinHTML=gold1gTaskbarPins().map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff()).map(a=>`<button class="taskbar-app pinned" data-appid="${esc(a.id)}" title="Pinned: ${esc(a.name)}" onclick="Gold50.openApp('${esc(a.id)}')">${appIcon(a,true)}<span>${esc(a.name)}</span></button>`).join('');
...
}catch(e){console.warn('Gold 1G taskbar override skipped',e)}
try{
  renderStartMenu=function(){
    const sm=$('startMenu'); if(!sm) return;
    const apps=visibleApps(); const groups=[...new Set(apps.map(a=>a.group))];
    const pins=gold1gStartPins().map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff());
...
}catch(e){console.warn('Gold 1G Start override skipped',e)}
function gold1gContextMenu(items,x,y){
...
function gold1gDesktopContext(e){
  ...
  if(fileEl){
    ...
    return gold1gContextMenu([
      {id:'open',label:'Open',action:()=>openFile(fid)},
      {id:'unpin',label:'Unpin file from desktop',action:()=>gold1gUnpinFileFromDesktop(fid)},
  ...
  if(appEl){
    ...
    return gold1gContextMenu([
      ...
      {id:'start',label:gold1gStartPins().includes(id)?'Unpin from Start':'Pin to Start',action:()=>gold1gStartPins().includes(id)?gold1gUnpinAppFromStart(id):gold1gPinAppToStart(id)},
      {id:'taskbar',label:gold1gTaskbarPins().includes(id)?'Unpin from taskbar':'Pin to taskbar',action:()=>gold1gTaskbarPins().includes(id)?gold1gUnpinAppFromTaskbar(id):gold1gPinAppToTaskbar(id)},
      {id:'desktop',label:(p.desktopApps||[]).includes(id)?'Remove from desktop':'Pin to desktop',action:()=>((prefs().desktopApps||[]).includes(id)?gold1gRemoveAppFromDesktop(id):gold1gPinAppToDesktop(id))},
      'sep',
  ...
  return gold1gContextMenu([
    ...
    'sep',
    {id:'clean',label:'Use essential desktop layout',action:()=>gold1gApplyEssentialDesktop()},
    {id:'settings',label:'Settings',action:()=>openSettings()}
  ],e.clientX,e.clientY);
}
function openDesktopCustomizer1B(){
  const p=prefs(); const pinnedFiles=gold1gDesktopPins(); const sPins=gold1gStartPins(); const tPins=gold1gTaskbarPins();
  openWindow('desktopcustomizer','Desktop Customizer',`<div class="goldshell-header"><div><h2>Desktop Customizer</h2><p class="muted">Classic EmeraldOS right-click tools for desktop apps, Start pins, taskbar pins, and pinned files.</p></div><button class="btn primary" onclick="Gold50.gold1gApplyEssentialDesktop()">Essential desktop</button></div><div class="grid2"><div class="card"><h3>Apps</h3><div class="purpose-map compact">${visibleApps().map(a=>`<div class="card mini"><h4>${appIcon(a,true)} ${esc(a.name)}</h4><div class="toolbar"><button class="btn small" onclick="Gold50.${(p.desktopApps||[]).includes(a.id)?'gold1gRemoveAppFromDesktop':'gold1gPinAppToDesktop'}('${esc(a.id)}')">${(p.desktopApps||[]).includes(a.id)?'Remove desktop':'Desktop'}</button><button class="btn small" onclick="Gold50.${sPins.includes(a.id)?'gold1gUnpinAppFromStart':'gold1gPinAppToStart'}('${esc(a.id)}')">${sPins.includes(a.id)?'Unpin Start':'Start'}</button><button class="btn small" onclick="Gold50.${tPins.includes(a.id)?'gold1gUnpinAppFromTaskbar':'gold1gPinAppToTaskbar'}('${esc(a.id)}')">${tPins.includes(a.id)?'Unpin taskbar':'Taskbar'}</button></div></div>`).join('')}</div></div><div class="card"><h3>Files pinned to desktop</h3><p class="muted">Pin important files from your Gold VM to the desktop without moving them.</p>${files().filter(f=>!f.trash).slice(0,80).map(f=>`<div class="settings-row"><div><b>${esc(f.name)}</b><p class="muted">${esc(f.folder)} · ${esc(f.type)}</p></div><button class="btn small" onclick="Gold50.${pinnedFiles.includes(f.id)?'gold1gUnpinFileFromDesktop':'gold1gPinFileToDesktop'}('${esc(f.id)}')">${pinnedFiles.includes(f.id)?'Unpin':'Pin file'}</button></div>`).join('')||'<p class="muted">No files yet.</p>'}</div></div>`,{width:1100,height:720})
}
function openUpdateSetup1B(){
  const box=$('setupWizard'); if(!box) return;
  let step=0;
  const steps=[
    ()=>`<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>EmeraldOS Gold 1G Update Setup</h2><p class="muted">Your VM was updated through E.L.S.U.S. This short setup keeps your files and settings, then helps you choose the new desktop layout.</p></div></div><div class="setup-checklist"><span>VM preserved</span><span>Desktop controls added</span><span>Classic right-click returned</span></div></div>`,
    ()=>`<h2>Choose your updated desktop</h2><p class="muted">Gold 1G keeps only essential apps on the desktop by default. Everything else stays available in Start and Search.</p><div class="grid3"><button class="tile-button" onclick="Gold50.gold1gApplyEssentialDesktop()">Use essential desktop</button><button class="tile-button" onclick="Gold50.openDesktopCustomizer1B()">Customize pins</button><button class="tile-button" onclick="Gold50.openApp('themestudio')">Customize style</button></div><h3>Essential desktop apps</h3><div class="grid3">${GOLD1G_ESSENTIAL_DESKTOP.map(id=>appById(id)).filter(Boolean).map(a=>`<div class="card">${appIcon(a,true)} <b>${esc(a.name)}</b><p class="muted">${esc(a.desc)}</p></div>`).join('')}</div>`,
    ...
    ()=>`<h2>You're ready</h2><p class="muted">Gold 1G is ready. Your VM data is still managed by E.L.S.U.S. and your existing data has been kept.</p><div class="grid3"><button class="tile-button" onclick="Gold50.openApp('updateshell')">System Update</button><button class="tile-button" onclick="Gold50.openApp('desktopcustomizer')">Desktop Customizer</button><button class="tile-button" onclick="Gold50.openApp('support')">Support Center</button></div>`
  ];
  function render(){box.classList.remove('hidden');box.innerHTML=`<div class="setup-card setup-card-rebuilt update-setup"><div class="setup-titlebar"><b>Gold 1G Update Setup</b><button class="btn small" onclick="Gold50.finishUpdateSetup1B()">Skip</button></div><div class="setup-steps">${steps.map((_,i)=>`<div class="setup-dot ${i<=step?'active':''}"></div>`).join('')}</div><div class="setup-body">${steps[step]()}</div><div class="toolbar setup-actions"><button class="btn" ${step===0?'disabled':''} onclick="Gold50.updateSetupStep1B(-1)">Back</button><span class="muted">Step ${step+1} of ${steps.length}</span><button class="btn primary" onclick="Gold50.updateSetupStep1B(1)">${step===steps.length-1?'Finish':'Next'}</button></div></div>`}
...
function finishUpdateSetup1B(){localStorage.setItem(PREFIX+'update_setup_done','true');$('setupWizard')?.classList.add('hidden');notify('Update setup complete','EmeraldOS Gold 1G is ready.','Setup')}
function gold1gInstallUpdateSetup(){
  if(gold1gShouldRunUpdateSetup()) setTimeout(openUpdateSetup1G,700);
}
function gold1gInstallContextMenus(){document.addEventListener('contextmenu',gold1gDesktopContext);document.addEventListener('keydown',e=>{if(e.shiftKey&&e.key==='F10'){e.preventDefault();const d=$('desktop');const r=d?.getBoundingClientRect();gold1gContextMenu([{id:'customize',label:'Open Desktop Customizer',action:()=>openDesktopCustomizer1B()},{id:'refresh',label:'Refresh desktop',action:()=>renderDesktop()}],(r?.left||20)+80,(r?.top||20)+80)}})}
gold1gAddOrUpdateApp({id:'desktopcustomizer',name:'Desktop Customizer',label:'DC',color:'blue',group:'Customization',purpose:'Desktop pin management',desc:'Classic desktop right-click tools for app pins, taskbar pins, Start pins, and pinned files.',open:openDesktopCustomizer1B});
Object.assign(window.Gold50||{}, {gold1gApplyEssentialDesktop,gold1gPinAppToDesktop,gold1gRemoveAppFromDesktop,gold1gPinAppToStart,gold1gUnpinAppFromStart,gold1gPinAppToTaskbar,gold1gUnpinAppFromTaskbar,gold1gPinFileToDesktop,gold1gUnpinFileFromDesktop,openDesktopCustomizer1B,openUpdateSetup1B,finishUpdateSetup1B});
window.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{try{gold1gInstallContextMenus();renderDesktop();renderStartMenu();renderTaskApps();gold1gInstallUpdateSetup();}catch(e){console.warn('Gold 1G polish failed',e)}},650)});


/* =========================================================
   EMERALDOS GOLD 1G - ADVANCED DESKTOP, ROLLBACK, CRASH UX,
   AND STAFF-ONLY E.L.S.U.S. PUBLISHER MANAGER
========================================================= */
const GOLD1G_PUBLISHER_PIN = "093013";
const GOLD1G_ROLLBACK_TARGETS = [
...
function gold1gValidColor(value, fallback){
  value=String(value||"").trim();
  return /^#[0-9a-f]{3,8}$/i.test(value)?value:fallback;
}
function gold1gNumber(value,min,max,fallback){
  value=Number(value);
...
try{
  const gold1gBaseDefaultPrefs = defaultPrefs;
  defaultPrefs = function(){
    const base = gold1gBaseDefaultPrefs();
    return {
      ...base,
...
}catch(e){console.warn("Gold 1G default preference extension skipped",e)}

try{
  const gold1gBaseApplyPrefs = applyPrefs;
  applyPrefs = function(){
    gold1gBaseApplyPrefs();
    const p=prefs();
    const root=document.documentElement;
    root.style.setProperty("--gold1gTaskbarColor", gold1gValidColor(p.taskbarColor,"#f6f9fd"));
    root.style.setProperty("--gold1gTaskbarText", gold1gValidColor(p.taskbarTextColor,"#111827"));
    root.style.setProperty("--gold1gStartColor", gold1gValidColor(p.startMenuColor,"#ffffff"));
    root.style.setProperty("--gold1gGridSize", gold1gNumber(p.desktopGridSize,64,150,92)+"px");
    document.body.classList.toggle("gold1g-free-desktop", p.desktopObjectMode!=="grid");
    document.body.classList.toggle("gold1g-grid-desktop", p.desktopObjectMode==="grid");
    document.body.classList.toggle("gold1g-trash-hidden", p.trashOnDesktop===false);
  };
}catch(e){console.warn("Gold 1G visual preference extension skipped",e)}

function gold1gDesktopPositions(){return read(PREFIX+"desktopPositions",{})}
function gold1gSaveDesktopPositions(map){write(PREFIX+"desktopPositions",map||{});saveWorkspaceDebounced()}
function gold1gDesktopItemKey(type,id){return type+":"+id}
function gold1gDefaultPosition(i){
  const p=prefs(), grid=gold1gNumber(p.desktopGridSize,64,150,92);
  const cols=Math.max(1,Math.floor((window.innerWidth-40)/grid));
  return {x:18+(i%cols)*grid, y:18+Math.floor(i/cols)*(grid+18)};
}
function gold1gPositionStyle(key,index){
  const pos=gold1gDesktopPositions()[key] || gold1gDefaultPosition(index);
  const x=gold1gNumber(pos.x,0,Math.max(0,innerWidth-92),18);
  const y=gold1gNumber(pos.y,0,Math.max(0,innerHeight-120),18);
  return `left:${x}px;top:${y}px;`;
}
function gold1gSnap(v){
  const p=prefs();
  if(!p.desktopSnapToGrid) return v;
  const grid=gold1gNumber(p.desktopGridSize,64,150,92);
  return Math.round(v/grid)*grid+18;
}
function gold1gTrashIconHTML(index){
  const key=gold1gDesktopItemKey('system','trash');
  return `<button class="desktop-icon desktop-trash ${prefs().showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-trash="true" style="${gold1gPositionStyle(key,index)}" ondblclick="Gold50.openTrashCan1G()" title="Trash"><div class="trash-can-icon">♻</div><span>Trash</span></button>`;
}
function openTrashCan1G(){
  try{ openExplorer(); setTimeout(()=>Gold50.explorerView('trash'),80); }
  catch(e){ gold1gFileOrAppFailure('Trash','Trash failed to load.',e,'trash'); }
}
...
try{
  renderDesktop=function(){
    const d=$('desktop'); if(!d) return;
    const p=prefs();
    d.classList.add('gold1g-movable-desktop');
    d.style.gap=p.density==='compact'?'8px':p.density==='spacious'?'22px':'16px';
    ...
    const appHTML=(p.desktopApps||[]).map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff()).map(a=>{
      const key=gold1gDesktopItemKey('app',a.id);
      const html=`<button class="desktop-icon ${p.showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-appid="${esc(a.id)}" style="width:${width};${gold1gPositionStyle(key,i++)}" ondblclick="Gold50.openApp('${esc(a.id)}')" title="${esc(a.desc)}">${appIcon(a)}<span>${esc(a.name)}</span></button>`;
      return html;
    }).join('');
    const fileHTML=gold1gDesktopPins().map(fid=>files().find(f=>f.id===fid&&!f.trash)).filter(Boolean).map(f=>{
      const key=gold1gDesktopItemKey('file',f.id);
      return `<button class="desktop-icon desktop-file ${p.showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-fileid="${esc(f.id)}" style="width:${width};${gold1gPositionStyle(key,i++)}" ondblclick="Gold50.openFile('${esc(f.id)}')" title="${esc(f.name)}"><div class="file-desktop-icon">${esc(fileIcon(f.type))}</div><span>${esc(f.name)}</span></button>`;
    }).join('');
    const trashHTML=p.trashOnDesktop===false?'':gold1gTrashIconHTML(i++);
    d.innerHTML=appHTML+fileHTML+trashHTML;
  };
}catch(e){console.warn('Gold 1G desktop renderer failed to install',e)}

function gold1gInstallDesktopDrag(){
  let state=null;
  ...
  document.addEventListener('pointerup',e=>{
    ...
    state.icon.style.left=x+'px'; state.icon.style.top=y+'px';
    const map=gold1gDesktopPositions(); map[state.key]={x,y,updatedAt:now()}; gold1gSaveDesktopPositions(map);
...
function gold1gCenterPopup(title,message,level='warning',actions=[]){
  if(prefs().centerFailurePopups===false) return notify(title,message,'EmeraldOS Gold');
  document.querySelectorAll('.gold1g-center-popup').forEach(x=>x.remove());
  const d=document.createElement('div');
  d.className='gold1g-center-popup '+level;
...
function gold1gFileOrAppFailure(name,message,error,kind='app'){
  console.warn('Gold 1G load failure',name,error);
  notify('Load problem',`${name} refused to load.`,'EmeraldOS Gold');
  gold1gCenterPopup(`${kind==='file'?'File':'Application'} refused to load`,`${message} ${error?.message||''}`.trim(),'warning',[
...
function gold1gBSOD(reason='EmeraldOS Gold stopped responding.',code='GOLD_RUNTIME_FAULT'){
  if(document.querySelector('.gold1g-bsod')) return;
  const d=document.createElement('div');
  d.className='gold1g-bsod';
  d.innerHTML=`<div><h1>:(</h1><h2>EmeraldOS Gold ran into a problem.</h2><p>${esc(reason)}</p><p class="bsod-code">Stop code: ${esc(code)}</p><div class="bsod-progress"><span></span></div><div class="toolbar"><button onclick="location.reload()">Restart EmeraldOS Gold</button><button onclick="localStorage.setItem('${PREFIX}safemode','true');location.reload()">Restart in Safe Mode</button><button onclick="this.closest('.gold1g-bsod').remove();Gold50.openApp('support')">Open Support</button></div></div>`;
  document.body.appendChild(d);
}

try{
  const gold1gOriginalOpenApp = window.Gold50?.openApp || openApp;
  function gold1gSafeOpenApp(id){
    try{
      const app=appById(id);
      if(!app) throw new Error('The app is not installed or is hidden.');
      return gold1gOriginalOpenApp(id);
    }catch(e){gold1gFileOrAppFailure(id,'EmeraldOS Gold could not start this app.',e,'app');return false;}
  }
  window.Gold50.openApp=gold1gSafeOpenApp;
  openApp=gold1gSafeOpenApp;
}catch(e){console.warn('Gold 1G app failure wrapper skipped',e)}
try{
  const gold1gOriginalOpenFile = window.Gold50?.openFile || openFile;
  function gold1gSafeOpenFile(fid){
    try{
      ...
      return gold1gOriginalOpenFile(fid);
    }catch(e){gold1gFileOrAppFailure(fid,'EmeraldOS Gold could not open this file.',e,'file');return false;}
  }
  window.Gold50.openFile=gold1gSafeOpenFile;
  openFile=gold1gSafeOpenFile;
}catch(e){console.warn('Gold 1G file failure wrapper skipped',e)}

function gold1gReleaseManifest(){
  return {
    product:'EmeraldOS Gold', latestVersion:'1G', build:'1G', folder:'Gold_1G', entry:'OS.html',
    channel:'stable', status:'stable', required:false, enabled:true, setupMode:'migrationIntro',
    releaseTitle:'EmeraldOS Gold 1G',
    summary:'Gold 1G adds deeper customization, movable desktop objects, Trash, rollback tools, centered load-failure popups, crash recovery screens, and staff/PIN-gated E.L.S.U.S. publishing.',
    migrationFrom:['1B','1A','11.0'], migrationId:'gold1b-to-gold1g-desktop-rollback',
    minShellVersion:'1.0', rollbackFolder:'Gold_1B', rollbackVersion:'1B', releasedAt:new Date().toISOString()
  };
}
function gold1gPublisherUnlocked(){return localStorage.getItem(PREFIX+'publisher_pin_unlocked')==='true'}
async function gold1gRequirePublisherAccess(show=true){
  if(!isStaff()){
    if(show) gold1gCenterPopup('Staff verification required','Update publishing is only available after signing in through Gold Staff Edition.','danger',[{label:'Open Staff Edition',primary:true,action:()=>location.href='staff.html'}]);
    return false;
  }
  if(gold1gPublisherUnlocked()) return true;
  const pin=prompt('Update Publisher Manager PIN');
  if(pin!==GOLD1G_PUBLISHER_PIN){
    if(show) gold1gCenterPopup('Publisher PIN rejected','The Update Publisher Manager PIN was incorrect.','danger');
...
async function gold1gPublishLatest(showResult=true){
  if(!(await gold1gRequirePublisherAccess(showResult))) return false;
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
    const manifest=gold1gReleaseManifest();
    ...
    if(showResult) gold1gCenterPopup('E.L.S.U.S. published','system/emeraldGoldLatest now points to Gold_1G. Users will still update manually when prompted.','success');
    return true;
  }catch(e){
    console.warn('Gold 1G publish failed',e);
    if(showResult) gold1gCenterPopup('Publish failed','Firebase rejected the publish request. Check Firestore rules and staff write permissions.','danger',[{label:'Open Diagnostics',action:()=>Gold50.openApp('diagnostics')}]);
    return false;
  }
}

function openELSUSUpdateCenter1G(){
  const latest=read('emeraldGoldShell_latest',gold1gReleaseManifest());
  const update=read('emeraldGold_updateAvailable',{});
  openWindow('updateshell','E.L.S.U.S. System Update',`<div class="goldshell-header"><div><h2>E.L.S.U.S. System Update</h2><p class="muted">The update shell stays separate. This app shows update status and lets users restart through E.L.S.U.S. Publishing is staff-only.</p></div><button class="btn primary" onclick="location.href='../gold-shell.html?manualCheck=1'">Check through shell</button></div><div class="grid2"><div class="card"><h3>This VM folder</h3><p>EmeraldOS Gold 1G</p><p class="muted">Gold_1G / OS.html</p></div><div class="card"><h3>Latest known folder</h3><p>${esc(latest.latestVersion||'Unknown')}</p><p class="muted">${esc(latest.folder||'No folder')} / ${esc(latest.entry||'OS.html')}</p></div><div class="card"><h3>User-controlled updates</h3><p>Each user updates manually. Their active folder changes only after accepting the update prompt.</p></div><div class="card"><h3>Rollback</h3><p>Rollback can help recover from a bad update, but may cause app or file instability.</p><button class="btn" onclick="Gold50.openApp('rollback')">Open Rollback</button></div></div><h3>Actions</h3><div class="toolbar"><button class="btn primary" onclick="Gold50.saveWorkspaceNow(true)">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html?manualCheck=1'">Restart through E.L.S.U.S.</button>${isStaff()?'<button class="btn danger" onclick="Gold50.openUpdatePublisherManager1G()">Update Publisher Manager</button>':''}</div><p class="muted">Last shell check: ${esc(update.checkedAt||'Not checked in this browser')}</p>`,{width:1080,height:740});
}
function openUpdatePublisherManager1G(){
  if(!isStaff()){gold1gStaffGateNotice();return}
  gold1gRequirePublisherAccess(true).then(ok=>{
    if(!ok)return;
    const hist=read(PREFIX+'publish_history',[]);
    openWindow('updatepublisher','Update Publisher Manager',`<div class="goldshell-header"><div><h2>Update Publisher Manager</h2><p class="muted">Staff-only E.L.S.U.S. publishing. A publisher PIN is required every session.</p></div><button class="btn danger" onclick="localStorage.removeItem('${PREFIX}publisher_pin_unlocked');Gold50.notify('Publisher locked','PIN access was cleared.','E.L.S.U.S.')">Lock publisher</button></div><div class="grid2"><div class="card"><h3>Publish Gold 1G</h3><p>Writes <code>system/emeraldGoldLatest</code> to point users to <code>Gold_1G/OS.html</code>. Users must still accept the update manually.</p><button class="btn primary" onclick="Gold50.gold1gPublishLatest(true)">Publish Gold 1G as latest</button></div><div class="card"><h3>Rollback target</h3><p>Default rollback: Gold 1B. Changing the public latest version should only be done if the current latest build breaks boot.</p><button class="btn" onclick="Gold50.gold1gPublishRollbackLatest('Gold_1B','1B')">Publish Gold 1B as emergency latest</button></div><div class="card"><h3>PIN status</h3><p>${gold1gPublisherUnlocked()?'Unlocked':'Locked'}</p></div><div class="card"><h3>Staff session</h3><p>${esc(username())}</p><p class="muted">Role: ${esc(currentStaff()?.role||localStorage.getItem('role')||'staff')}</p></div></div><h3>Recent publishing</h3>${hist.map(h=>`<div class="card"><b>${esc(h.manifest?.releaseTitle||'Publish')}</b><p class="muted">${esc(h.time)} · ${esc(h.user)}</p></div>`).join('')||'<p class="muted">No publish history on this device.</p>'}`,{width:1080,height:760});
  });
}
async function gold1gPublishRollbackLatest(folder='Gold_1B',version='1B'){
  if(!(await gold1gRequirePublisherAccess(true)))return false;
  ...
  try{
    ...
    const manifest={...gold1gReleaseManifest(), latestVersion:version, build:version, folder, releaseTitle:`EmeraldOS Gold ${version} Emergency Rollback`, summary:'Emergency rollback published by staff through Gold 1G Update Publisher Manager.', rollbackFolder:'Gold_1G', rollbackVersion:'1G', releasedAt:new Date().toISOString()};
    await api.setDoc(api.doc(api.db,'system','emeraldGoldLatest'),manifest,{merge:true});
    gold1gCenterPopup('Emergency rollback published',`E.L.S.U.S. now points to ${folder}.`,'success');
    return true;
  }catch(e){gold1gCenterPopup('Rollback publish failed',e.message||'Firebase rejected the rollback publish.','danger');return false;}
}

function openRollbackCenter1G(){
  openWindow('rollback','Version Rollback',`<div class="goldshell-header"><div><h2>Version Rollback</h2><p class="muted">Rollback changes only this user's selected E.L.S.U.S. folder unless staff publishes an emergency rollback.</p></div><button class="btn primary" onclick="Gold50.saveVMSnapshot()">Create snapshot</button></div><div class="danger-card"><h3>Rollback warning</h3><p>Rolling back can cause instability with newer applications, files, settings, or VM data. Create a snapshot first and only rollback if a new version is not working.</p></div><div class="grid2">${GOLD1G_ROLLBACK_TARGETS.map(t=>`<div class="card"><h3>${esc(t.title)}</h3><p class="muted">${esc(t.folder)} / ${esc(t.entry)}</p><button class="btn" onclick="Gold50.gold1gUserRollback('${esc(t.folder)}','${esc(t.version)}')">Rollback my VM to ${esc(t.version)}</button></div>`).join('')}</div>`,{width:1000,height:720});
}
function gold1gUserRollback(folder,version){
  ...
  const manifest={product:'EmeraldOS Gold',latestVersion:version,build:version,folder,entry:'OS.html',channel:'stable',status:'rollback',required:false,enabled:true,setupMode:'continue',releaseTitle:'EmeraldOS Gold '+version,summary:'User selected rollback through Gold 1G.',rollbackFrom:'Gold_1G',rolledBackAt:now()};
  ...
  gold1gCenterPopup('Rollback selected','E.L.S.U.S. will restart into '+folder+'.','warning',[{label:'Restart now',primary:true,action:()=>location.href='../gold-shell.html?rollback=1'}]);
}

function openThemeStudio1G(){
  const p=prefs();
  openWindow('themestudio','Theme Studio',`<h2>Theme Studio</h2><p class="muted">Gold 1G adds safer deep customization with protected CSS variables.</p><div class="grid2"><div class="card"><h3>Colors</h3><label>Accent<input type="color" class="field" value="${esc(p.accent||'#0078d4')}" oninput="Gold50.setPrefs({accent:this.value})"></label><label>Secondary accent<input type="color" class="field" value="${esc(p.secondaryAccent||'#0f7b4d')}" oninput="Gold50.setPrefs({secondaryAccent:this.value})"></label><label>Taskbar color<input type="color" class="field" value="${esc(p.taskbarColor||'#f6f9fd')}" oninput="Gold50.gold1gSetPref('taskbarColor',this.value)"></label><label>Taskbar text<input type="color" class="field" value="${esc(p.taskbarTextColor||'#111827')}" oninput="Gold50.gold1gSetPref('taskbarTextColor',this.value)"></label><label>Start menu color<input type="color" class="field" value="${esc(p.startMenuColor||'#ffffff')}" oninput="Gold50.gold1gSetPref('startMenuColor',this.value)"></label></div><div class="card"><h3>Desktop objects</h3><label>Desktop mode<select class="field" onchange="Gold50.gold1gSetPref('desktopObjectMode',this.value)"><option value="free" ${p.desktopObjectMode!=='grid'?'selected':''}>Free move</option><option value="grid" ${p.desktopObjectMode==='grid'?'selected':''}>Grid layout</option></select></label><label><input type="checkbox" ${p.desktopSnapToGrid!==false?'checked':''} onchange="Gold50.gold1gSetPref('desktopSnapToGrid',this.checked)"> Snap objects to grid</label><label>Grid size<input type="range" min="64" max="150" value="${esc(p.desktopGridSize||92)}" oninput="Gold50.gold1gSetPref('desktopGridSize',this.value)"></label><label><input type="checkbox" ${p.trashOnDesktop!==false?'checked':''} onchange="Gold50.gold1gSetPref('trashOnDesktop',this.checked)"> Show Trash on desktop</label><button class="btn" onclick="Gold50.gold1gResetDesktopPositions()">Reset icon positions</button></div><div class="card"><h3>Background</h3><label>Background color<input type="color" class="field" value="${esc(p.backgroundColor||'#eaf3ff')}" oninput="Gold50.setGold70Pref?.('backgroundColor',this.value);Gold50.setGold70Pref?.('useBackgroundColor',true);Gold50.gold1gSetPref('backgroundColor',this.value)"></label><label>Background URL<input class="field" value="${esc(p.customBackground||'')}" onchange="Gold50.setPrefs({customBackground:this.value})"></label><input type="file" class="field" accept="image/*" onchange="Gold50.loadBackgroundFile(this)"></div><div class="card"><h3>Reliability</h3><label><input type="checkbox" ${p.centerFailurePopups!==false?'checked':''} onchange="Gold50.gold1gSetPref('centerFailurePopups',this.checked)"> Center failure popups</label><label><input type="checkbox" ${p.bsodOnCriticalErrors!==false?'checked':''} onchange="Gold50.gold1gSetPref('bsodOnCriticalErrors',this.checked)"> Crash screen for critical errors</label><button class="btn danger" onclick="Gold50.gold1gBSOD('Manual crash screen test.','MANUAL_TEST')">Test crash screen</button></div></div>`,{width:1080,height:760});
}
function gold1gSetPref(key,value){const p={};p[key]=value;setPrefs(p)}
function gold1gResetDesktopPositions(){localStorage.removeItem(PREFIX+'desktopPositions');renderDesktop();notify('Desktop reset','Desktop object positions were reset.','Desktop')}

function gold1gInstallTrashContext(){
  document.addEventListener('contextmenu',e=>{
    const trash=e.target.closest('[data-trash]'); if(!trash)return;
    e.preventDefault(); e.stopPropagation();
    gold1gContextMenu([
      {id:'open',label:'Open Trash',action:()=>openTrashCan1G()},
      ...
      {id:'hide',label:'Hide Trash icon',action:()=>gold1gSetPref('trashOnDesktop',false)}
    ],e.clientX,e.clientY);
  },true);
}
...
window.addEventListener('error',e=>{
  try{
    ...
    gold1gCenterPopup('EmeraldOS Gold warning',msg,'warning',[{label:'Open Diagnostics',action:()=>Gold50.openApp('diagnostics')}]);
    if(prefs().bsodOnCriticalErrors && /ReferenceError|SyntaxError|not defined|Cannot read|Cannot set/.test(msg)) setTimeout(()=>gold1gBSOD(msg,'GOLD_UNHANDLED_ERROR'),250);
  }catch{}
});
...
function gold1gInstallApps(){
  gold1gAddOrUpdateApp({id:'updateshell',name:'E.L.S.U.S. System Update',label:'UP',color:'blue',group:'System',purpose:'User update status',desc:'Check update status, save the VM, restart through E.L.S.U.S., and open rollback tools.',open:openELSUSUpdateCenter1G});
  gold1gAddOrUpdateApp({id:'updatepublisher',name:'Update Publisher Manager',label:'PM',color:'red',group:'Staff',purpose:'Staff update publishing',desc:'Staff-only and PIN-gated E.L.S.U.S. publishing controls.',staffOnly:true,open:openUpdatePublisherManager1G});
  gold1gAddOrUpdateApp({id:'rollback',name:'Version Rollback',label:'RB',color:'orange',group:'System',purpose:'Rollback',desc:'Rollback this VM to a previous EmeraldOS Gold folder with instability warnings.',open:openRollbackCenter1G});
  gold1gAddOrUpdateApp({id:'crashrecovery',name:'Crash Recovery',label:'CR',color:'blue',group:'Support',purpose:'Reliability',desc:'Test and recover from EmeraldOS Gold crash screens and load failures.',open:()=>openWindow('crashrecovery','Crash Recovery',`<h2>Crash Recovery</h2><p class="muted">Use these tools if an app or file refuses to load.</p><div class="grid3"><button class="tile-button" onclick="Gold50.gold1gBSOD('Manual crash screen test.','MANUAL_TEST')">Test crash screen</button><button class="tile-button" onclick="Gold50.openApp('diagnostics')">Diagnostics</button><button class="tile-button" onclick="localStorage.setItem('${PREFIX}safemode','true');location.reload()">Restart in Safe Mode</button></div>`,{width:780,height:520})});
  try{const theme=APPS.find(a=>a.id==='themestudio'); if(theme) theme.open=openThemeStudio1G;}catch{}
}

Object.assign(window.Gold50||{}, {
  openTrashCan1G,gold1gCenterPopup,gold1gBSOD,gold1gPublishLatest,gold1gRequirePublisherAccess,
  openELSUSUpdateCenter1G,openUpdatePublisherManager1G,gold1gPublishRollbackLatest,openRollbackCenter1G,gold1gUserRollback,
  openThemeStudio1G,gold1gSetPref,gold1gResetDesktopPositions
});
window.Gold1G=window.Gold50;
// Query-string publisher opening disabled in Gold 1G.
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1gInstallApps();gold1gInstallDesktopDrag();gold1gInstallTrashContext();applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1G feature install failed',e)}},900));


/* =========================================================
   EMERALDOS GOLD 1G FINAL UX, LANGUAGE, ACCESSIBILITY,
   STAFF DIRECT CONTROL, AND SAFE CUSTOMIZATION LAYER
========================================================= */
const gold1gBaseDefaultPrefs = defaultPrefs;
defaultPrefs = function(){
  return {
    ...gold1gBaseDefaultPrefs(),
    language:"en",
    taskbarColor:"#f6f9fd",
...
applyPrefs = function(){
  ...
  const p=prefs();
  const root=document.documentElement;
  root.style.setProperty('--gold1gTaskbarColor',p.taskbarColor||'#f6f9fd');
  root.style.setProperty('--gold1gTaskbarText',p.taskbarTextColor||'#111827');
  root.style.setProperty('--gold1gStartColor',p.startMenuColor||'#ffffff');
  root.style.setProperty('--gold1gBackgroundColor',p.backgroundColor||'#eaf3ff');
  root.style.setProperty('--gold1gPanelOpacity',String(Math.max(.55,Math.min(1,Number(p.panelOpacity||.96)))));
  root.style.setProperty('--gold1gMenuOpacity',String(Math.max(.55,Math.min(1,Number(p.menuOpacity||.96)))));
  document.documentElement.lang=p.language==='es'?'es':'en';
  document.body.dataset.language=p.language==='es'?'es':'en';
  document.body.classList.toggle('gold1g-readable',!!p.readableFont);
  document.body.classList.toggle('gold1g-hit-large',!!p.largerHitTargets);
  document.body.classList.toggle('gold1g-underline-controls',!!p.underlineControls);
  document.body.classList.toggle('gold1g-extra-contrast',!!p.highContrastBorders);
  document.body.classList.remove('gold1g-filter-warm','gold1g-filter-cool','gold1g-filter-gray');
  if(p.colorFilter&&p.colorFilter!=='none') document.body.classList.add('gold1g-filter-'+p.colorFilter);
...
function gold1gMarkProcessed(cmd,session){
...
const gold1gBaseProcessRemoteCommand = processRemoteCommand;
processRemoteCommand = function(cmd,session){
  ...
  if(!directTypes.includes(t)) return gold1gBaseProcessRemoteCommand(cmd,session);
  if(!gold1gMarkProcessed(cmd,session)) return;
  try{
...
const gold1gBaseSnapshot = remoteDesktopSnapshot;
remoteDesktopSnapshot = function(){
  const snap=gold1gBaseSnapshot();
...
function gold1gInstallFinalApps(){
  gold1gAddOrUpdateApp({id:'language',name:'Language & Region',label:'LR',color:'teal',group:'Settings',purpose:'Language',desc:'Switch EmeraldOS Gold between English and Spanish core shell language.',open:openLanguageRegion1G});
  gold1gAddOrUpdateApp({id:'accessibility',name:'Accessibility Center',label:'AC',color:'blue',group:'Settings',purpose:'Accessibility',desc:'Readable text, contrast, reduced motion, focus rings, and language access.',open:openAccessibility1G});
  gold1gAddOrUpdateApp({id:'staffresources',name:'Staff Resources',label:'SR',color:'purple',group:'Staff',purpose:'Staff resources',desc:'Staff-only support procedures, E.L.S.U.S. notes, and remote support policy.',staffOnly:true,open:openStaffResources1G});
  try{const t=APPS.find(a=>a.id==='themestudio'); if(t)t.open=openThemeStudio1GPlus;}catch{}
  try{const s=APPS.find(a=>a.id==='staff'); if(s)s.staffOnly=true;}catch{}
}
Object.assign(window.Gold50||{}, {openLanguageRegion1G,openAccessibility1G,openThemeStudio1GPlus,openStaffResources1G,gold1gT});
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1gInstallFinalApps();applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1G final layer failed',e)}},1050));




/* =========================================================
   EMERALDOS GOLD 1G - LICENSED SETUP, TASK MANAGER,
   STAFF MAIL PORTAL, GAMES, APP-SPECIFIC LOGOS, SAFE UI
========================================================= */
(function gold1gMigrateFrom1D(){
  try{
    if(localStorage.getItem('gold1g_migrated_from_gold1d')==='true') return;
    const oldPrefix='gold1d_', newPrefix='gold1g_';
    Object.keys(localStorage).forEach(k=>{
      if(k.startsWith(oldPrefix)){
        const nk=newPrefix+k.slice(oldPrefix.length);
        if(localStorage.getItem(nk)===null) localStorage.setItem(nk, localStorage.getItem(k));
    ...
    localStorage.setItem('gold1g_migrated_from_gold1d','true');
    if(localStorage.getItem(newPrefix+'setup_done')==='true') localStorage.setItem(newPrefix+'updated_from','1D');
  }catch(e){console.warn('Gold 1G migration from 1D skipped',e)}
})();
const GOLD1G_TOS_URL='https://securly-plans.github.io/TOS.html';
const GOLD1G_PUBLISHER_PIN_FINAL='093013';
...
function openSetup(){
  ...
  const steps=[
    ()=>`<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>Welcome to EmeraldOS Gold 1G</h2><p class="muted">Set up your EmeraldOS Gold virtual desktop once. Your setup, apps, files, preferences, support history, and VM state can be restored when you return.</p></div></div><div class="setup-checklist"><span>Cloud VM ready</span><span>Login protected</span><span>E.L.S.U.S. compatible</span></div></div>`,
    ()=>gold1gLicenseHTML('first'),
    ...
    ()=>`<h2>Ready to start</h2><p>EmeraldOS Gold 1G will save this setup to your VM profile.</p><div class="card"><h3>Virtual license</h3><p class="muted">Agreement will be recorded for this VM account and version.</p></div>`
  ];
  function render(){box.classList.remove('hidden'); const kind='first'; box.innerHTML=`<div class="setup-card setup-card-rebuilt gold1g-setup"><div class="setup-titlebar"><b>EmeraldOS Gold 1G First-Boot Setup</b></div><div class="setup-steps">${steps.map((_,i)=>`<div class="setup-dot ${i<=step?'active':''}"></div>`).join('')}</div><div class="setup-body">${steps[step]()}</div><div class="toolbar setup-actions"><button class="btn" ${step===0?'disabled':''} onclick="Gold50.gold1gSetupStep(-1)">Back</button><span class="muted">Step ${step+1} of ${steps.length}</span><button class="btn primary" ${step===1&&!gold1gCanAdvanceSetup(kind)?'disabled':''} onclick="Gold50.gold1gSetupStep(1)">${step===steps.length-1?'Accept and finish':'Next'}</button></div></div>`}
  window.Gold50.gold1gSetupStep=(d)=>{ if(step===1 && d>0 && !gold1gCanAdvanceSetup('first')){notify('Agreement required','You must agree to the Emerald Systems TOS and virtual license to continue.','Setup');return;} if(step+d>=steps.length){finishSetup();return;} step=Math.max(0,Math.min(steps.length-1,step+d));render(); };
  window.Gold50.gold1gToggleSetupApp=(id,on)=>{on?selected.add(id):selected.delete(id);setPrefs({desktopApps:[...selected]})};
  render();
}
const gold1gBaseFinishSetup = typeof finishSetup==='function'?finishSetup:null;
finishSetup=function(){
  ...
  localStorage.setItem(PREFIX+'setup_done','true');
  setElsusFirstSetupDone();
  setElsusLastVersion(BUILD.version);
  localStorage.setItem(PREFIX+'license_version','Gold_1G');
  localStorage.setItem(PREFIX+'license_time',now());
  $('setupWizard')?.classList.add('hidden');
  notify('Setup complete','EmeraldOS Gold 1G is ready.','Setup');
  saveWorkspaceNow?.(true);
};
function openUpdateSetup1G(){
  const box=$('setupWizard'); if(!box) return; let step=0;
  const steps=[
    ()=>`<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>EmeraldOS Gold 1G Update Setup</h2><p class="muted">Your VM updated through E.L.S.U.S. This short setup preserves your files and preferences while reviewing new terms and features.</p></div></div><div class="setup-checklist"><span>VM preserved</span><span>New app logos</span><span>Task Manager added</span></div></div>`,
    ()=>gold1gLicenseHTML('update'),
    ()=>`<h2>What changed in Gold 1G?</h2><div class="grid2"><div class="card"><h3>Setup and license</h3><p>First-Boot Setup and Update Setup now include the EmeraldOS Gold virtual license and TOS agreement.</p></div><div class="card"><h3>Apps</h3><p>Task Manager, Staff Mail Portal, and Gold Games were added without duplicating existing app purposes.</p></div><div class="card"><h3>Login and staff access</h3><p>Gold still requires normal login. Staff resources and apps require Staff Edition verification.</p></div><div class="card"><h3>Design</h3><p>App logos and shell surfaces were refreshed for a cleaner modern EmeraldOS Gold desktop.</p></div></div>`,
  ...
  function render(){box.classList.remove('hidden');box.innerHTML=`<div class="setup-card setup-card-rebuilt update-setup gold1g-setup"><div class="setup-titlebar"><b>Gold 1G Update Setup</b><button class="btn small" onclick="Gold50.finishUpdateSetup1G()">Skip after agreement</button></div><div class="setup-steps">${steps.map((_,i)=>`<div class="setup-dot ${i<=step?'active':''}"></div>`).join('')}</div><div class="setup-body">${steps[step]()}</div><div class="toolbar setup-actions"><button class="btn" ${step===0?'disabled':''} onclick="Gold50.updateSetupStep1G(-1)">Back</button><span class="muted">Step ${step+1} of ${steps.length}</span><button class="btn primary" ${step===1&&!gold1gCanAdvanceSetup('update')?'disabled':''} onclick="Gold50.updateSetupStep1G(1)">${step===steps.length-1?'Finish update setup':'Next'}</button></div></div>`}
  window.Gold50.updateSetupStep1G=(d)=>{ if(step===1 && d>0 && !gold1gCanAdvanceSetup('update')){notify('Agreement required','You must agree to the TOS and virtual license to continue.','Update Setup');return;} if(step+d>=steps.length){finishUpdateSetup1G();return;} step=Math.max(0,Math.min(steps.length-1,step+d));render(); };
  render();
}
function finishUpdateSetup1G(){
  ...
  localStorage.setItem(PREFIX+'license_time',now());
  $('setupWizard')?.classList.add('hidden');
  notify('Update setup complete','EmeraldOS Gold 1G is ready.','Update Setup');
  saveWorkspaceNow?.(true);
}
function gold1gInstallUpdateSetup(){
  if(gold1gShouldRunUpdateSetup()) setTimeout(openUpdateSetup1G,700);
}
function openTaskManager1G(){
...
function openStaffMailPortal1G(){
  if(!isStaff()){notify('Staff login required','Open Staff Edition and verify your staff account to access Staff Mail Portal.','Staff'); gold1gStaffGateNotice(); return;}
...
function openGoldGames1G(){
  openWindow('games','Gold Games',`<h2>Gold Games</h2><p class="muted">Built-in offline games for EmeraldOS Gold. Scores stay inside your VM profile.</p><div class="games-grid"><div class="game-card"><h3>Gold Clicker</h3><p>Simple score challenge.</p><button class="btn primary" onclick="Gold50.openGameClicker1G()">Play</button></div><div class="game-card"><h3>Memory Match</h3><p>Match Emerald cards.</p><button class="btn primary" onclick="Gold50.openGameMemory1G()">Play</button></div><div class="game-card"><h3>Gold Blocks</h3><p>Mini block counter puzzle.</p><button class="btn primary" onclick="Gold50.openGameBlocks1G()">Play</button></div></div>`,{width:820,height:560});
}
function openGameClicker1G(){let score=Number(localStorage.getItem(PREFIX+'clicker_score')||0);openWindow('game_clicker','Gold Clicker',`<h2>Gold Clicker</h2><div class="clicker-score" id="clicker_score">${score}</div><button class="btn primary big-game-btn" onclick="Gold50.gameClick1G()">Collect Gold</button><button class="btn" onclick="localStorage.setItem('${PREFIX}clicker_score','0');Gold50.openGameClicker1G()">Reset</button>`,{width:520,height:420})}
function gameClick1G(){let s=Number(localStorage.getItem(PREFIX+'clicker_score')||0)+1;localStorage.setItem(PREFIX+'clicker_score',String(s));const el=$('clicker_score');if(el)el.textContent=s;}
function openGameMemory1G(){const symbols=['G','E','★','◆','G','E','★','◆'].sort(()=>Math.random()-.5);write(PREFIX+'memory_board',symbols.map((s,i)=>({i,s,open:false,done:false})));write(PREFIX+'memory_pick',[]);renderMemory1G()}
function renderMemory1G(){const b=read(PREFIX+'memory_board',[]);openWindow('game_memory','Memory Match',`<h2>Memory Match</h2><div class="memory-board">${b.map(c=>`<button class="memory-card ${c.open||c.done?'open':''}" onclick="Gold50.memoryPick1G(${c.i})">${c.open||c.done?esc(c.s):'?'}</button>`).join('')}</div><button class="btn" onclick="Gold50.openGameMemory1G()">New game</button>`,{width:560,height:520})}
function memoryPick1G(i){let b=read(PREFIX+'memory_board',[]),p=read(PREFIX+'memory_pick',[]);const c=b.find(x=>x.i===i);if(!c||c.done||c.open)return;c.open=true;p.push(i);if(p.length===2){const [a,bid]=p.map(id=>b.find(x=>x.i===id));if(a&&bid&&a.s===bid.s){a.done=bid.done=true}else setTimeout(()=>{let nb=read(PREFIX+'memory_board',[]);nb.forEach(x=>{if(!x.done)x.open=false});write(PREFIX+'memory_board',nb);renderMemory1G()},700);p=[]}write(PREFIX+'memory_board',b);write(PREFIX+'memory_pick',p);renderMemory1G()}
function openGameBlocks1G(){const state=read(PREFIX+'blocks_state',{turns:12,score:0});openWindow('game_blocks','Gold Blocks',`<h2>Gold Blocks</h2><p class="muted">Pick blocks. Gold gives points, gray costs a turn.</p><div class="taskmgr-stats"><div><b>${state.score}</b><span>Score</span></div><div><b>${state.turns}</b><span>Turns</span></div></div><div class="blocks-board">${Array.from({length:12},(_,i)=>`<button class="block" onclick="Gold50.blocksPick1G(${i})">■</button>`).join('')}</div><button class="btn" onclick="localStorage.removeItem('${PREFIX}blocks_state');Gold50.openGameBlocks1G()">Reset</button>`,{width:560,height:520})}
function blocksPick1G(i){let s=read(PREFIX+'blocks_state',{turns:12,score:0});if(s.turns<=0)return; s.turns--; s.score+=Math.random()>.35?10:-3; write(PREFIX+'blocks_state',s); if(s.turns<=0)notify('Gold Blocks finished','Final score: '+s.score,'Gold Games'); openGameBlocks1G()}
function openUpdatePublisherManager1G(){
  if(!isStaff()){notify('Staff login required','You must be logged into Staff Edition to access Update Publisher Manager.','System Update'); gold1gStaffGateNotice(); return;}
  const unlocked=localStorage.getItem(PREFIX+'publisher_unlocked')==='true';
  openWindow('updatepublisher','Update Publisher Manager',`<h2>Update Publisher Manager</h2><p class="muted">Staff-only E.L.S.U.S. release publishing. Requires Staff Edition and the publisher PIN.</p>${!unlocked?`<div class="card"><h3>PIN required</h3><input class="field" id="publisher_pin" type="password" placeholder="Publisher PIN"><button class="btn primary" onclick="Gold50.unlockPublisher1G(document.getElementById('publisher_pin').value)">Unlock</button></div>`:`<div class="card success-card"><h3>Publisher unlocked</h3><p>Gold 1G can now publish its Firebase latest pointer for this staff session.</p></div><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1gPublishLatest(true)">Publish Gold 1G as latest</button><button class="btn" onclick="Gold50.gold1gPublishRollback1D()">Emergency rollback to Gold 1D</button></div>`}<div class="card"><h3>Current manifest</h3><pre>${esc(JSON.stringify(GOLD1G_MANIFEST,null,2))}</pre></div>`,{width:940,height:720});
}
function unlockPublisher1G(pin){if(String(pin||'')===GOLD1G_PUBLISHER_PIN_FINAL){localStorage.setItem(PREFIX+'publisher_unlocked','true');notify('Publisher unlocked','Update Publisher Manager is available for this staff session.','System Update');openUpdatePublisherManager1G()}else notify('Incorrect PIN','The Update Publisher Manager PIN was not accepted.','System Update')}
async function gold1gPublishLatest(showResult=true){
  if(!isStaff()||localStorage.getItem(PREFIX+'publisher_unlocked')!=='true') {openUpdatePublisherManager1G();return false;}
  try{const api=fb||await initFirebase(); if(!api) throw new Error('Firebase module did not load.'); await api.setDoc(api.doc(api.db,'system','emeraldGoldLatest'), GOLD1G_MANIFEST,{merge:true}); localStorage.setItem(PREFIX+'latest_manifest_published','true'); write('emeraldGoldShell_latest',GOLD1G_MANIFEST); if(showResult)notify('E.L.S.U.S. updated','system/emeraldGoldLatest now points to Gold_1G.','System Update'); return true;}catch(e){console.warn('Gold 1G publish failed',e); if(showResult)notify('Publish failed',e.message,'System Update'); return false;}
}
async function gold1gPublishRollback1D(){
  if(!isStaff()||localStorage.getItem(PREFIX+'publisher_unlocked')!=='true') return openUpdatePublisherManager1G();
  const rollback={...GOLD1G_MANIFEST,latestVersion:'1D',build:'1D',folder:'Gold_1D',status:'rollback',releaseTitle:'Emergency rollback to EmeraldOS Gold 1D',summary:'Staff emergency rollback from Gold 1G.',rollbackFrom:'Gold_1G',rolledBackAt:now()};
...
function gold1gInstallApps(){
  gold1gUpsertApp({id:'taskmanager',name:'Task Manager',label:'TM',color:'blue',group:'System',purpose:'Process management',desc:'View open windows, local VM storage, session state, and end tasks.',open:openTaskManager1G});
  gold1gUpsertApp({id:'staffmail',name:'Staff Mail Portal',label:'SM',color:'teal',group:'Staff',purpose:'Staff webmail portal',desc:'Staff-only Emerald Mail portal through mail.monroecorp.org.',staffOnly:true,open:openStaffMailPortal1G});
  gold1gUpsertApp({id:'games',name:'Gold Games',label:'GG',color:'gold',group:'Games',purpose:'Built-in games',desc:'Offline EmeraldOS Gold games hub.',open:openGoldGames1G});
  gold1gUpsertApp({id:'updatepublisher',name:'Update Publisher Manager',label:'UP',color:'red',group:'Staff',purpose:'Staff E.L.S.U.S. publishing',desc:'Staff-only update publishing with publisher PIN.',staffOnly:true,open:openUpdatePublisherManager1G});
...
// Tighten publish functions from earlier layers so publishing is staff + PIN only in Gold 1G.
gold1gPublishLatestManifest = gold1gPublishLatest;
gold1gPublishLatest = gold1gPublishLatest;
function gold1gSafeRenderAll(){try{applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1G render refresh failed',e)}}
Object.assign(window.Gold50||{}, {gold1gLicenseText,gold1gLicenseHTML,gold1gCopyLicense,gold1gSetLicenseAgreement,gold1gSetupStep:null,openUpdateSetup1G,finishUpdateSetup1G,updateSetupStep1G:null,openTaskManager1G,openStaffMailPortal1G,openGoldGames1G,openGameClicker1G,gameClick1G,openGameMemory1G,memoryPick1G,openGameBlocks1G,blocksPick1G,openUpdatePublisherManager1G,unlockPublisher1G,gold1gPublishLatest,gold1gPublishRollback1D});
window.Gold1G=window.Gold50;
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1gInstallApps();gold1gInstallUpdateSetup();gold1gSafeRenderAll();}catch(e){console.warn('Gold 1G layer failed',e)}},1300));
// Gold 1G does not auto-publish from query strings. Publishing is Staff Edition + Update Publisher Manager + PIN only.


/* =========================================================
   GOLD 1G FINAL E.L.S.U.S. VM CONTINUITY PATCH
   - First-Boot Setup is global per user across all E.L.S.U.S. versions.
   - Update Setup remains per-version.
...
async function gold1gCloudReadSetupState(){
  try{
    const api = fb || await initFirebase();
    if(!api?.getDoc || !api?.doc || !api?.db) return null;
    const snap = await api.getDoc(api.doc(api.db,'emeraldOSUsers',username(),'goldVM','setup'));
    return snap.exists() ? snap.data() : null;
  }catch(error){
    console.warn('Gold 1G cloud setup read failed', error);
    return null;
  }
}
async function gold1gCloudWriteSetupState(type){
  const data = {
    firstBootCompleted: elsusFirstSetupDone(),
    firstBootSetupDone: elsusFirstSetupDone(),
    firstBootScope: 'global-elsus-user',
    lastCompletedVersion: BUILD.version,
    lastUpdateSetupVersion: type === 'update' ? BUILD.version : (localStorage.getItem(elsusUserKey(ELSUS_VERSION_BASE)) || BUILD.version),
    lastSetupType: type,
    licenseAccepted: true,
    licenseVersion: 'Gold_1G',
    tosUrl: GOLD1G_TOS_URL,
    folder: 'Gold_1G',
    updatedAt: now()
  };
  try{
    const api = fb || await initFirebase();
    if(!api?.setDoc || !api?.doc || !api?.db) return false;
    await api.setDoc(api.doc(api.db,'emeraldOSUsers',username(),'goldVM','setup'), data, {merge:true});
    return true;
  }catch(error){
    console.warn('Gold 1G cloud setup write failed', error);
    return false;
  }
}
async function gold1gLoadCloudSetupState(){
  const data = await gold1gCloudReadSetupState();
  if(data?.firstBootCompleted || data?.firstBootSetupDone){
    setElsusFirstSetupDone();
    localStorage.setItem(PREFIX+'setup_done','true');
    if(data.lastCompletedVersion) setElsusLastVersion(data.lastCompletedVersion);
  }
  if(data?.lastUpdateSetupVersion === BUILD.version){
    localStorage.setItem(PREFIX+'update_setup_done','true');
  }
  return data;
}
async function gold1gCloudAwareBootSetupGate(){
  try{ await gold1gLoadCloudSetupState(); }catch(error){ console.warn('Gold 1G setup cloud gate failed', error); }
  gold1gBootSetupGate();
}
const gold1gFinalFinishSetup = finishSetup;
finishSetup = function(){
  gold1gFinalFinishSetup();
  if(localStorage.getItem(PREFIX+'setup_done')==='true'){
    setElsusFirstSetupDone();
    setElsusLastVersion(BUILD.version);
    gold1gCloudWriteSetupState('first');
  }
};
const gold1gFinalFinishUpdateSetup = finishUpdateSetup1G;
finishUpdateSetup1G = function(){
  gold1gFinalFinishUpdateSetup();
  if(localStorage.getItem(PREFIX+'update_setup_done')==='true'){
    setElsusFirstSetupDone();
    setElsusLastVersion(BUILD.version);
    gold1gCloudWriteSetupState('update');
  }
};
async function gold1gSaveCloudVMState(show=true){
  ...
  data.lastSave = now();
  write(PREFIX+'workspace', data);
  try{
    const api = fb || await initFirebase();
    if(api?.setDoc && api?.doc && api?.db){
      await api.setDoc(api.doc(api.db,'emeraldOSUsers',username(),'goldVM','current'), data, {merge:true});
      localStorage.setItem(PREFIX+'cloud_status','saved');
      localStorage.setItem(PREFIX+'cloud_last_save',now());
      if(show) notify('Cloud VM saved','Your EmeraldOS Gold VM state was saved to goldVM/current.','Gold VM');
      return data;
    }
  }catch(error){ console.warn('Gold 1G VM cloud save failed', error); }
  if(show) notify('Saved locally','Cloud VM save will retry when Firebase is available.','Gold VM');
  return data;
}
async function gold1gRestoreCloudVMState(show=true){
  let data = null;
  try{
    const api = fb || await initFirebase();
    if(api?.getDoc && api?.doc && api?.db){
      const snap = await api.getDoc(api.doc(api.db,'emeraldOSUsers',username(),'goldVM','current'));
      if(snap.exists()) data = snap.data();
    }
  }catch(error){ console.warn('Gold 1G VM cloud restore failed', error); }
  if(!data) data = read(PREFIX+'workspace', null);
  if(data){
    if(data.localKeys) Object.entries(data.localKeys).forEach(([k,v])=>{ if(v!==null && v!==undefined) localStorage.setItem(k,String(v)); });
    ['prefs','files','tickets','mail','notes','tasks','contacts','notifications'].forEach(k=>{ if(data[k]) write(PREFIX+k,data[k]); });
    if(data.userApps) write(PREFIX+'user_apps', data.userApps);
    if(data.remoteSessions) write(PREFIX+'remote_sessions', data.remoteSessions);
    if(data.emergencyLogs) write(PREFIX+'emergency_logs', data.emergencyLogs);
    if(data.snapshots) write(PREFIX+'vm_snapshots', data.snapshots);
    applyPrefs(); renderDesktop(); renderStartMenu(); renderNotifBadge();
    if(show) notify('Cloud VM restored','Your EmeraldOS Gold desktop was restored from the version-independent VM profile.','Gold VM');
  }else if(show){
    notify('No cloud VM state found','First-Boot Setup will create your EmeraldOS Gold VM profile.','Gold VM');
  }
}
saveWorkspaceNow = gold1gSaveCloudVMState;
restoreWorkspace = gold1gRestoreCloudVMState;
Object.assign(window.Gold50||{}, {
  saveWorkspaceNow,
  restoreWorkspace,
